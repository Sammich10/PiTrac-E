...
/**
 * \brief The GolfBall class represents a golf ball in the simulation. It contains properties and methods
...
public:
    ...
    /*
    TODO: In the long term, need to figure out how to manage x, y, radius, and the ball circle so that
    ...
    */
     ...
     */
    cv::Point2f center() const { return cv::Point2f(x_, y_); }
    ...
    cv::Scalar average_color_;                  // RGB Triplet (stored in BGR order as per openCV)
    cv::Scalar median_color_;               // RGB Triplet (stored in BGR order as per openCV)
    cv::Scalar std_color_;                  // Triplet (stored in BGR order as per openCV)
