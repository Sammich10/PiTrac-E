...
#ifndef PULSE_STROBE_H
#define PULSE_STROBE_H
...
class PulseStrobe
{
  ...
  protected:
    ...
    // This vector describes the amount of time to send 0's after sending a
    // strobe
    // pulse.  The last pulse should be of size 0 to ensure the pulse sequence
    // ends
    ...
    // This is the buffer that will be written out (bit-banged) to the SPI
    // channel
    static char *camera_slow_pulse_sequence_;
    static char *camera_fast_pulse_sequence_;
    static char *no_pulse_camera_sequence_;  // Will be all 0's, but the same
                                             // length as the 'real' pulse
                                             // sequence
