...
bool GolfSimOptions::Parse(int argc, char *argv[])
{
    ...
    artifact_save_level_ =
        (ArtifactSaveLevel)artifact_save_level_table[artifact_save_level_string_];
...
void GolfSimOptions::Print() const
{
    ...
    if (camera_gain_ > 0)
    {
        std::cout << "    camera_gain (will override .json file settings): " <<
            std::to_string(camera_gain_) << std::endl;
    }
}
} // namespace PiTrac
