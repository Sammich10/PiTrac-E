...
enum SystemMode
{
    kTest = 0,              // Just run unit tests.  Generally used when
                            // compiled in Windows
    kCamera1 = 1,           // The mode when PiTrac is running normally and
                            // processing hits.  This mode is only used by the
                            // Pi 1/Camera 1 half of the system.
    kCamera2 = 2,           // The mode when PiTrac is running normally and
                            // processing hits.  This mode is only used by the
                            // Pi 2/Camera 2 half of the system.
    ...
    kRunCam2ProcessForPi1Processing = 15,      // This is for when a process is
                                               // running on camera 2 for the
                                               // purpose of auto-calibration or
                                               // taking pictures for ball
                                               // location
...
{
    kNoArtifacts = 0,
    kFinalResultsOnly = 1,      // produces images, but only at a few
                                // higher-level points in the processing
    kAll = 2                    // May slow the system to a crawl, with over a
                                // dozen large intermediate files being written
};
...
struct GolfSimOptions
{
    GolfSimOptions()
    {
        ...
        ("golfer_orientation",
         value<std::string>(&golfer_orientation_string_)->default_value("right_handed"),
        ...
        ("artifact_save_level",
         value<std::string>(&artifact_save_level_string_)->default_value("final_results_only"),
        ...
        ("cam_still_mode",
         value<bool>(&camera_still_mode_)->default_value(false)->implicit_value(true),
         "Take a single camera2 still picture (using one strobe flash) and exit")
        ("lm_comparison_mode",
         value<bool>(&lm_comparison_mode_)->default_value(false)->implicit_value(true),
         "Configure for operating in another infrared-based LM environment")
        ("send_test_results",
         value<bool>(&send_test_results_)->default_value(false)->implicit_value(true),
        ...
        ("pulse_test",
         value<bool>(&perform_pulse_test_)->default_value(false)->implicit_value(true),
        ...
        ("wait_keys",
         value<bool>(&wait_for_key_on_images_)->default_value(false)->implicit_value(true),
        ...
        ("use_non_IR_camera",
         value<bool>(&use_non_IR_camera_)->default_value(false)->implicit_value(true),
        ...
        ("simulate_found_ball",
         value<bool>(&simulate_found_ball_)->default_value(false)->implicit_value(true),
        ...
        ("config_file", value<std::string>(&config_file_)->default_value("PiTrac_config.json"),
         "Specify the filename with the JSON configuration.  Default is: PiTrac_config.json")
    ...
    double camera_gain_ = 0.0;       // 1.0 might seem more appropriate, but we
                                     // want to be able to see if this is set or
                                     // not
