...
// This class provides an interface to the .json configuration file that is used
// to set
// various constants in the system.  It is also responsible for reading many of
// those
...
class GolfSimConfiguration
{
  public:
    ...
    (
        const std::string &configuration_filename = "gs_config.json"
    );

    // Uses a safer version of getenv when in Windows environment.
    static std::string safe_getenv
    (
    ...
    // Reads any values that need to be initialized early, such as static
    // members of
    // classes that won't otherwise have a good place to be otherwise
    // initialized because,
    ...
    static bool ReadShotInjectionData
    (
        std::vector<GsResults> &shots,
        int &kInterShotInjectionPauseSeconds
    );

    // Set the specified value to the value.  The node will be created if
    // necessary
    static bool SetTreeValue
    (
        const std::string &tag_name,
        const cv::Vec2d &vec
    );
