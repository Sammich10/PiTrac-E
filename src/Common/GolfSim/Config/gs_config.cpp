...
// Having to set the constants in this way creates more entanglement than we'd
// like.  TBD - Re-architect
...
bool GolfSimConfiguration::Initialize(const std::string &configuration_filename)
{
    ...
    {
        GS_LOG_MSG(error,
                   "GolfSimConfiguration::Initialize failed. ERROR: *** " + std::string(e.what()) +
                   " ***");
        return false;
    }
...
{
    try {
        SetConstant("gs_config.testing.kInterShotInjectionPauseSeconds",
                    kInterShotInjectionPauseSeconds);

        // Retrirve as many shots as are defined in the json file
        boost::property_tree::ptree shots_json =
            configuration_root_.get_child("gs_config.testing.test_shots_to_inject");

        int shot_number = 1;
        for (boost::property_tree::ptree::iterator iter = shots_json.begin();
             iter != shots_json.end();
             iter++)
        {
    ...
    {
        GS_LOG_MSG(error,
                   "GolfSimConfiguration::ReadShotInjectionData failed. ERROR: *** " +
                   std::string(e.what()) + " ***");
        return false;
    }
    return true;
...
GolfSimConfiguration::PiModel GolfSimConfiguration::GetPiModel()
{
    ...
    GS_LOG_TRACE_MSG(trace,
                     "GolfSimConfiguration - bcm_host_get_processor_id returned:" +
                     std::to_string(processor_type));
...
{
    // Many constants are read in the modules that own those constants.  But
    // some are better
    // initialized here, early, before those modules may get to be initialized
    // (if they even
    // have an initialization).

    SetConstant("gs_config.physical_constants.kBallRadiusMeters", GolfBall::kBallRadiusMeters);

    SetConstant("gs_config.cameras.kCamera1PositionsFromExpectedBallMeters",
                GolfSimCamera::kCamera1PositionsFromExpectedBallMeters);
    SetConstant("gs_config.cameras.kCamera2PositionsFromExpectedBallMeters",
                GolfSimCamera::kCamera2PositionsFromExpectedBallMeters);
    SetConstant("gs_config.cameras.kCamera2OffsetFromCamera1OriginMeters",
                GolfSimCamera::kCamera2OffsetFromCamera1OriginMeters);

#ifdef __unix__  // Ignore in Windows environment

    SetConstant("gs_config.user_interface.kWebServerResultBallExposureCandidates",
                GsUISystem::kWebServerResultBallExposureCandidates);
    SetConstant("gs_config.user_interface.kWebServerResultSpinBall1Image",
                GsUISystem::kWebServerResultSpinBall1Image);
    SetConstant("gs_config.user_interface.kWebServerResultSpinBall2Image",
                GsUISystem::kWebServerResultSpinBall2Image);
    SetConstant("gs_config.user_interface.kWebServerResultBallRotatedByBestAngles",
                GsUISystem::kWebServerResultBallRotatedByBestAngles);
    SetConstant("gs_config.user_interface.kWebServerErrorExposuresImage",
                GsUISystem::kWebServerErrorExposuresImage);
    SetConstant("gs_config.user_interface.kWebServerBallSearchAreaImage",
                GsUISystem::kWebServerBallSearchAreaImage);

    SetConstant("gs_config.image_capture.kMaxWatchingCropWidth",
                LibCameraInterface::kMaxWatchingCropWidth);
    SetConstant("gs_config.image_capture.kMaxWatchingCropHeight",
                LibCameraInterface::kMaxWatchingCropHeight);
    ...
    // Let the command-line gain parameter override the .json config file
    // parameter
    // TBD - May want to have separate gain options?
    if (GolfSimOptions::GetCommandLineOptions().camera_gain_ > 0.0)
    {
        GS_LOG_MSG(info,
                   "Overriding camera gains with value: " +
                   std::to_string(GolfSimOptions::GetCommandLineOptions().camera_gain_) + ".");
    ...
    SetConstant("gs_config.cameras.kCamera2CalibrateOrLocationGain",
                LibCameraInterface::kCamera2CalibrateOrLocationGain);
    SetConstant("gs_config.cameras.kCamera2ComparisonGain",
                LibCameraInterface::kCamera2ComparisonGain);
    SetConstant("gs_config.testing.kCamera2StrobedEnvironmentGain",
                LibCameraInterface::kCamera2StrobedEnvironmentGain);
    ...
    SetConstant("gs_config.cameras.kCamera2PuttingContrast",
                LibCameraInterface::kCamera2PuttingContrast);
    SetConstant("gs_config.cameras.kCamera1StillShutterTimeuS",
                LibCameraInterface::kCamera1StillShutterTimeuS);
    SetConstant("gs_config.cameras.kCamera2StillShutterTimeuS",
                LibCameraInterface::kCamera2StillShutterTimeuS);
    SetConstant("gs_config.cameras.kCameraMotionDetectSettings",
                LibCameraInterface::kCameraMotionDetectSettings);

    // The web server share directory isn't really a value we want to use from
    // the .json configuration
    // file anymore, but for now, let's allow it as a fall-back to the command
    // line
    if (!GolfSimOptions::GetCommandLineOptions().web_server_share_dir_.empty())
    {
        GsUISystem::kWebServerShareDirectory =
            GolfSimOptions::GetCommandLineOptions().web_server_share_dir_;
    }
    else
    {
        // Attempt to get the image logging directory from the .json config file
        SetConstant("gs_config.user_interface.kWebServerShareDirectory",
                    GsUISystem::kWebServerShareDirectory);
    }

    // If the configuration file forgot to add a "/" at the end of the logging
    // directory, we should add it here ourselves
    ...
    GS_LOG_TRACE_MSG(info,
                     "GolfSimConfiguration - PITRAC_SLOT1_CAMERA_TYPE environment variable was: " +
                     slot1_env );
    if (slot1_env.empty())
    {
        GS_LOG_TRACE_MSG(info,
                         "GolfSimConfiguration - PITRAC_SLOT1_CAMERA_TYPE environment variable was not set.  Assuming default of: "
                         + std::to_string(GolfSimCamera::kSystemSlot1CameraType));
    }
    else
    {
#ifndef __unix__  // Ignore in Windows environment
        // Ensure we don't have any trailing spaces.  Visual Studio seems to add
        // them?
    ...
    if (slot2_env.empty())
    {
        ...
        {
            // We somewhat arbitrarily require the slot2 camera type to be set
            // if we're running in single-pi mode.
            GS_LOG_TRACE_MSG(error,
                             "GolfSimConfiguration - PITRAC_SLOT2_CAMERA_TYPE environment variable must be set when running in single-pi mode, but was not.  Exiting.");
            return false;
        }
        else
        {
...
#endif
            GS_LOG_TRACE_MSG(info,
                             "GolfSimConfiguration - PITRAC_SLOT2_CAMERA_TYPE environment variable was not set.  Assuming default of: "
                             + std::to_string(GolfSimCamera::kSystemSlot2CameraType));
        }
    }
    else
    {
...
void GolfSimConfiguration::SetConstant(const std::string &tag_name, bool &constant_value)
{
    ...
    {
        GS_LOG_MSG(error,
                   "GolfSimConfiguration::SetConstant failed. ERROR: *** " + std::string(e.what()) +
                   " ***");
...
void GolfSimConfiguration::SetConstant(const std::string &tag_name, cv::Vec3d &vec)
{
    try {
        int i = 0;
        for (boost::property_tree::ptree::value_type &element :
             configuration_root_.get_child(tag_name))
        {
...
bool GolfSimConfiguration::RemoveTreeNode(const std::string &tag_name)
{
    ...
    {
        GS_LOG_MSG(error,
                   "GolfSimConfiguration::RemoveTreeNode failed. ERROR: *** " +
                   std::string(e.what()) + " ***");
        return false;
    }
...
bool GolfSimConfiguration::SetTreeValue(const std::string &tag_name, const cv::Vec2d &vec)
{
    ...
    {
        GS_LOG_MSG(error,
                   "GolfSimConfiguration::SetTreeValue failed. ERROR: *** " +
                   std::string(e.what()) + " ***");
        return false;
    }
...
bool GolfSimConfiguration::WriteTreeToFile(const std::string &file_name)
{
    ...
    {
        GS_LOG_MSG(error,
                   "GolfSimConfiguration::WriteTreeToFile failed. Could not open file for writing.");
        return false;
    }
...
std::string GolfSimConfiguration::GetSystemID()
{
    ...
    GS_LOG_MSG(trace,
               "GolfSimConfiguration::GetSystemID - run_single_pi = " +
               std::to_string(single_pi_mode) +
               ", cameraNumber = " +
               std::to_string(GolfSimOptions::GetCommandLineOptions().GetCameraNumber()) +
               ", system_mode_ = " +
    ...
    // Note that if the system is in still or auto-calibrate modes, this is
    // system 1, regardless of which
    // camera is going to take the picture.
    // In cases where the Pi 2/ Camera 2 system needs to take a picture, that
    // will happen in a separate
    // process that (as far as it knows) will run just like it does when
    // operating in the normal launch
    // monitor mode.
    if ((GolfSimOptions::GetCommandLineOptions().GetCameraNumber() == GsCameraNumber::kGsCamera1 &&
         (GolfSimOptions::GetCommandLineOptions().system_mode_ !=
          SystemMode::kRunCam2ProcessForPi1Processing)) ||
