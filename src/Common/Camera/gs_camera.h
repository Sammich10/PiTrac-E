 ...
 *  The module generally interfaces to the rest of the system by taking images
 * as input
 *  and by producing golf_ball objects as output.
 *
 *  An important function of this class is to identify a set of potential golf
 * balls (circles)
 *  from a strobed image that may include multiple, possibly-overlapping golf
 * balls.
...
struct GsBallPairAndSpinCandidateScoreElement
{
    ...
    // If the user's leg is likely to be where the ball is (depends on
    // handedness),
    // that's probably a worse ball
    double leg_proximity_score = 0;
    // The closer the two balls are in radii, the better the spin calculation
    // will
    // likely be.
...
class GolfSimCamera
{
  public:
    ...
    //FAKE_PHOTO = "./FakePiCameraPhotoOfGolfBall-Clr-White-Flat.png";   //
    // Having problems with this one
    static constexpr std::string_view FAKE_PHOTO =
    ...
    // This allows the calibration to work even if the ball is a little
    // off-center
    static float kTeedBallSearchAreaMaskRadiusRatio;

    // Once the ball has been moved (hit), we expect it's radius will be
    // relatively close to the original radius
    // where is started (or close) as a minimum, as well as a little larger or
    // smaller.
    // Max is a little larger to adjust for possible error and stretching out of
    // balls near the edge
    static float kMaxMovedBallRadiusRatio;
    static float kMinMovedBallRadiusRatio;

    // Determines whether and how intermediate processing images (of which there
    // are over a dozen) are logged to the
    ...
    // The remainder of these constants control the way that the (probably
    // overly-complicated) ball-identification
    // processing works.  Best to see how they are used in the rest of the code
    // to understand them.
    // These constants get their initial values from the configuration .json
    // file for the system.
    ...
    // Used generally to create a reasonable range around an expected or other
    // radius
    ...
    // overcome problems associated with using the LM with another (also
    // strobed)
    // LM
    ...
    // Here, the "1" refers not necessarily to the camera that is named camera 1
    // functionally
    // (the camera that watches the teed-up ball).
    static CameraHardware::CameraModel kSystemSlot1CameraType;
    // If this instance of the system only has a single camera, there may be no
    // type set for Camera 2.
    // Instead, another instance, presumably on another Pi, will have the other
    // camera
    static CameraHardware::CameraModel kSystemSlot2CameraType;

    // Refers to the camera_hardware device object associated with this
    // higher-level camera object
    ...
    // image by using various circle-identification algorithms and other
    // processing.
    //
    ...
    // TBD - Should return a vector of golf ball objects with each ball's
    // current information.  A vector could be returned to support, e.g.,
    // a multi-strobed picture where the camera shutter is open while the strobe
    // flashes multiple times.
    // Returns true iff the input ball was successfully calibrated
    // camera_positions_from_origin[0] is the position for the calibrated ball,
    // [1] is the position for the camera that took this image
    bool GetCurrentBallLocation
    (
        const GolfSimCamera &camera,
        const cv::Mat &img,
    ...
    // In some cases, camera_1 will be the same as for camera_2 (such as
    // comparing two strobed balls from camera 2).
    static bool ComputeBallDeltas
    (
        GolfBall &ball1,
        GolfBall &ball2,
    ...
    // This will reverse whatever values are necessary to "correct" the
    // processing for left-handed shots
    static bool ReverseBallAngleDeltas
    (
        GolfBall &ball
    );

    static bool ComputeSingleBallXYZOrthoCamPerspective
    (
        const GolfSimCamera &camera,
        GolfBall &initial_ball
    );

    // Use the specified time delay and the already-calculated deltas in the
    // ball to determine velocity
    static void CalculateBallVelocity
    (
        GolfBall &b,
        long time_delay_us
    );
    ...
    // Use the specified time delay and the already-calculated deltas in the
    // ball to determine spin rate
    // Also sets the rotation_results into the ball object
    static void CalculateBallSpinRates
    (
        GolfBall &b,
    ...
    bool AnalyzeStrobedBalls
    (
        const cv::Mat &strobed_balls_color_image,
    ...
    // Sets up the LoggingTool root cause and prints out an error if there are
    // less than two strobed balls found
    static void ReportBallSearchError
    (
        const int number_balls_found
    );

    static bool ShowAndLogBalls
    (
    ...
    bool GetBallDistancesAndRatios
    (
    ...
    // The ball closest to the center of the screen (assuming everything else is
    // ok with it) will
    // likely be the ball that will produce the best "face" for spin detection,
    // especially if compared
    // to the view that the first camera has of the initially-positioned ball.
    int GetMostCenteredBallIndex
    (
        const std::vector<GolfBall> &balls,
        const int ball_to_ignore_index = -1
    );

    // Sorts the ball vector by X positions.
    // Sorts with the "furthest" ball last, i.e., right-most-to-left for a
    // right-handed
    // golfer, and leftmost-to-rightmost for a left-handed golfer
    void SortBallsByXPosition
    (
        std::vector<GolfBall> &balls
    );

    // NOTE:  the hsv range may include negative or > 180 Hue numbers due to the
    // hue looping at 180 degrees
    static std::vector<cv::Scalar> GetBallHSVRange
    (
        const cv::Scalar &ball_color_RGB
    );

    // Somewhat of a pass-through for now, but we want to preserve a layer above
    // the hardware
    ...
    // The focal distance for each camera should already be known, but this
    // function allows a reverse
    // calculation that takes the meters and imaged ball radius and determines
    // the focal distance from that.
    static double computeFocalDistanceFromBallData
    (
        const GolfSimCamera &camera,
    ...
    // Returned angles (x,y) (degrees):  x is left or right angle to the
    // "origin" (default, at rest) ball.
    //     camera_positions_from_origin assume that the camera is aimed at the
    // origin.  That is, the origin will
    //          appear in the middle of the camera's view
    //     Input camera positions are in meters.
    //     Positive x angles mean the ball is positioned to the left of the
    // camera, looking out from the camera
    //         at the ball.
    //     Negative y angles mean the ball is below the camera's axis, looking
    // out at the ball from the camera
    cv::Vec2d computeCameraAnglesToBallPlane
    (
        cv::Vec3d &camera_positions_from_origin
    );

    // Accounts for the fact that the camera may not be pointing straight ahead
    bool AdjustXYZDistancesForCameraAngles
    (
        const cv::Vec2d &camera_angles,
        const cv::Vec3d &original_distances,
        cv::Vec3d &adjusted_distances
    );


    // Returns the difference, in meters, between b2 - b1 for the X, Y, and Z
    // axes
    static bool ComputeXyzDeltaDistances
    (
        const GolfBall &b1,
        const GolfBall &b2,
    ...
    // Returns the real-world distances in the X and Y directions from the
    // perspective of the camera,
    // based in part on the input Z-distance from the ball to the camera.  The
    // X,Y origin from which the
    // distances are measured is considered the center of the image.
    // Regardless of the camera angle, the returned distances are as if the
    // camera was facing straight at the
    // plane of the expected ball's line of flight.
    static bool ComputeXyzDistanceFromOrthoCamPerspective
    (
        const GolfSimCamera &camera,
        const GolfBall &b1,
    ...
    static bool ComputeBallXYAnglesFromCameraPerspective
    (
    ...
    // Requires the golf balls to have their distances_camera_perspective; and
    // angles_camera_perspective vector
    static bool getXYDeltaAnglesBallPerspective
    (
        const cv::Vec3d &position_deltas_ball_perspective,
        cv::Vec2d &deltaAnglesBallPerspective
    );

    // Given the current focal length, direct-to-ball distance, and sensed ball
    // radius, determine what a distance in pixels in the
    // X or Y direction is in real-life distance in meters.
    static double convertXDistanceToMeters
    (
        const GolfSimCamera &camera,
        double zDistanceMeters,
        double xDistancePixels
    );
    static double convertYDistanceToMeters
    (
        const GolfSimCamera &camera,
        double zDistanceMeters,
    ...
    // Given two images of a golf ball, with the first taken before the second,
    // this function
    // takes those images and the time delay between them to determine the
    // velocity and 3D spin
    // speed of the ball.
    ...
    (
        const std::vector<GolfBall> &input_balls,
        GolferOrientation &handedness
    );


    bool analyzeShotImages
    (
        const GolfSimCamera &camera,
        const cv::Mat &rgbImg1,
        const cv::Mat &rgbImg2,
        long timeDelayuS,
        const std::vector<cv::Vec3d> &camera_positions_from_origin,                           // Same
                                                                                              // X,Y,Z
                                                                                              // coordinate
                                                                                              // system
                                                                                              // as
    ...
    // Finds the current color information from the image at the point where the
    // ball exists and
    // sets up the corresponding color information
    static void GetBallColorInformation
    (
        const cv::Mat &color_image,
        GolfBall &b
    );

    // Because the strobe timing between different images of the ball can differ
    // from ball-pair to pair,
    // this method helps figure what those intervals are.
    bool DetermineStrobeIntervals
    (
    ...
    void PrintPulseVector
    (
        const std::vector<bool> &combinations_vector
    );
    ...
    int FindClosestRatioPatternMatchOffset
    (
        const std::vector<double> distance_ratios,
        const std::vector<double> pulse_ratios,
        double &delta_to_closest_ratio,
        bool try_different_offsets = true
    );

    bool GetPulseIntervalsAndRatiosFromIntervalVector
    (
    ...
    bool GetPulseIntervalsAndRatios
    (
    ...
    double GetPerpendicularDistanceFromLine
    (
        double x,
        double y,
        double x1,
    ...
    bool FindBestBallOnLineOfFlight
    (
        const std::vector<GolfBall> &balls,
    ...
    // Returns a score of the closeness of the vector of distance_ratios within
    // the pulse_ratios at an offset
    // of the distance_ratios from the beginning of the pulse_ratios
    double ComputeRatioDistance
    (
        const std::vector<double> distance_ratios,
        const std::vector<double> pulse_ratios,
    ...
    void RemoveLowScoringBalls
    (
        std::vector<GolfBall> &initial_balls,
        const int max_balls_to_retain
    );

    // Erases any balls whose distance (at a right angle) from the line between
    // the best and second-best ball
    // is greater than the max_distance_from_trajectory
    void RemoveOffTrajectoryBalls
    (
        std::vector<GolfBall> &initial_balls,
    ...
    void RemoveUnlikelyRadiusChangeBalls
    (
        std::vector<GolfBall> &initial_balls,
    ...
    void RemoveTooSmallOrBigBalls
    (
        std::vector<GolfBall> &initial_balls,
        const GolfBall &expected_best_ball
    );

    void RemoveWrongColorBalls
    (
        const cv::Mat &img,
        std::vector<GolfBall> &initial_balls,
        const GolfBall &expected_best_ball,
    ...
    void RemoveUnlikelyAngleLowerQualityBalls
    (
        std::vector<GolfBall> &initial_balls
    );

    void RemoveWrongRadiusBalls
    (
        std::vector<GolfBall> &initial_balls,
        const GolfBall &expected_best_ball
    );

    void RemoveNearbyPoorQualityBalls
    (
        std::vector<GolfBall> &initial_balls,
    ...
    // The img is used as a frame to ensure that the balls that are chosen are
    // not too close to the edges
    static bool FindClosestTwoBalls
    (
        const cv::Mat &img,
    ...
    static bool FindBestTwoSpinBalls
    (
        const cv::Mat &img,
        const GsBallsAndTimingVector &balls,
        const bool use_edge_backoffs,
    ...
    // For each pair of balls, determines the angles and velocity, and then
    // averages
    // all of them and returns that average in output_averaged_ball
    static bool ComputeAveragedStrobedBallData
    (
        const GolfSimCamera &camera,
    ...
    // Return_balls will hold the set of balls that are non-overlapping with
    // other balls
    // Will also remove/collapse pulse intervals as necessary to ensure they
    // stay
    // correlated wiht the return_balls vector.
    uint RemoveOverlappingBalls
    (
        const std::vector<GolfBall> &initial_balls,
    ...
    double AdjustDistanceForSlowing
    (
        const double initial_right_distance
    );

    // Analyze the ball exposures in the image and return ball2 with the
    // trajectory, spin, etc. information
    // exposures_image returns an image of the ball exposures that were
    // identified.
    static bool ProcessReceivedCam2Image
    (
        const cv::Mat &ball1_mat,
    ...
    static bool CleanExternalStrobeArtifacts
    (
        const cv::Mat &image,
        cv::Mat &output_image,
    ...
    static bool AutoCalibrateCamera
    (
        GsCameraNumber camera_number
    );

    static bool RetrieveAutoCalibrationConstants
    (
        GsCameraNumber camera_number
    );

    static bool DetermineCameraAngles
    (
        const cv::Mat &color_image,
        const GolfSimCamera &camera,
    ...
    // Take a single still picture with the specified camera.  May require the
    // Pi 2 (Camera 2)
    // process to be running if that is the specified camera.
    static bool TakeStillPicture
    (
        const GolfSimCamera &camera,
        cv::Mat &color_image
    );
    ...
    static double DetermineFocalLengthForAutoCalibration
    (
        const cv::Mat &color_image,
        const GolfSimCamera &camera
    );

    // Determine how much we widen the color mask from the average color of the
    // ball
    ...
    static constexpr float EXTRA_MAX_HUE_RANGE_RATIO = 1.1f;       // An extra
                                                                   // boost for
                                                                   // the HUE ?
                                                                   // TBD

    // Min values will be subtracted from the average HSV values, max will be
    // added
    static constexpr int H_MIN_CAL_COLOR_WIDENING_AMOUNT = 60;      // 12 worked
                                                                    // for green
                                                                    // ball, but
                                                                    // white
                                                                    // seems to
  ...
  private:
    ...
    // NOTE - getCameraParameters must already have been called before this
    // function is called
    static int getExpectedBallRadiusPixels
    (
        const CameraHardware &camera_hardware,
    ...
    // Compute the distance to the ball based on the known radius of the ball in
    // the real world
    static double ComputeDistanceToBallUsingRadius
    (
        const GolfSimCamera &camera,
        const GolfBall &ball
    );
