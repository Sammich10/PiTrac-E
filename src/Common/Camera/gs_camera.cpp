...
namespace PiTrac {
    ...
    std::vector<cv::Scalar> GolfSimCamera::GetBallHSVRange(const cv::Scalar& ball_color_RGB) {
        ...
        GS_LOG_TRACE_MSG(trace, "GetBallHSVRange for average (" + LoggingTools::Formatcv::Scalar(hsvAvg) + ") = "
            + LoggingTools::Formatcv::Scalar(hsvMin) + " | " + LoggingTools::Formatcv::Scalar(hsvMax));
    ...
    bool GolfSimCamera::ComputeSingleBallXYZOrthoCamPerspective(const GolfSimCamera& camera, GolfBall& ball1 ) {
        ...
        */
        ...
        void GolfSimCamera::RemoveWrongColorBalls(const cv::Mat& rgbImg, 
            ...
            std::vector<cv::Scalar> statistics = CvUtils::GetBallColorRgb(rgbImg, expected_best_ball.ball_circle_);
            ...
            for (int i = (int)initial_balls.size() - 1; i >= 0; i--) {
                ...
                GS_LOG_TRACE_MSG(trace, "\n\nExamining circle No. " + std::to_string(i) + ".  Radius " + std::to_string(b.measured_radius_pixels_) +
                    ...
                    + ". Average HSV is{ " + LoggingTools::Formatcv::Scalar(CvUtils::ConvertRgbToHsv(avg_RGB)));
        ...
        bool GolfSimCamera::ProcessReceivedCam2Image(const cv::Mat& ball1_mat,
            ...
            GolfSimConfiguration::SetConstant("gs_config.PiTraculator_interfaces.kSkipSpinCalculation", kSkipSpinCalculation);
        ...
        bool GolfSimCamera::CleanExternalStrobeArtifacts(const cv::Mat &image, cv::Mat& output_image, std::vector<cv::Vec4i>& lines)
        {
            ...
            /****
            cv::Scalar lowerHsv{ (float)kExternallyStrobedEnvFilterHsvLowerH, (float)kExternallyStrobedEnvFilterHsvLowerS, (float)kExternallyStrobedEnvFilterHsvLowerV };
            cv::Scalar upperHsv{ (float)kExternallyStrobedEnvFilterHsvUpperH, (float)kExternallyStrobedEnvFilterHsvUpperS, (float)kExternallyStrobedEnvFilterHsvUpperV };
        ...
        // TBD - MOVE to where the other parameters are.  This is just here for development convenience for now
        ...
        double GolfSimCamera::DetermineFocalLengthForAutoCalibration(const cv::Mat& color_image, const GolfSimCamera& camera) {
	    ...
	    if (kNumberPicturesForFocalLengthAverage > 0) {
            ...
            GS_LOG_TRACE_MSG(info, "Saving current PiTrac_config.json file to filename = " + backup_json_file_name);
            ...
            if (command_result != 0) {
                GS_LOG_TRACE_MSG(trace, "system(cp_command) failed. Could not backup existing PiTrac_config.json file.  Exiting");
                return false;
            }
