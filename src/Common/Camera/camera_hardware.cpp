...
namespace PiTrac {
    ...
    ) : camera_model_(m), camera_number_(n), focal_length_(focalLengthOverride), cameraInitialized(false) 
    {
    ...
    void CameraHardware::init_camera_parameters(const GsCameraNumber camera_number, const CameraModel model, const float focalLengthOverride) {

        GS_LOG_TRACE_MSG(trace, "Initializing camera parameters for camera number: " + std::to_string(camera_number) + " and model: " + camera_model_to_string(model));
