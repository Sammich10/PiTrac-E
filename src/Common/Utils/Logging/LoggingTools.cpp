...
#include <algorithm>
#include <format>
#include "Common/Utils/Logging/LoggingTools.h"
...
void LoggingTools::InitLogging()
{
    ...
    auto fmtTimeStamp = boost::log::expressions::
                        format_date_time<boost::posix_time::ptime>("TimeStamp",
                                                                   "%Y-%m-%d %H:%M:%S.%f");
...
// Get a window size (x, y) appropriate to the image, limited to something
// reasonable -- 600 pixels
cv::Vec2i LoggingTools::GetImageWindowSize(const cv::Mat &img)
{
    ...
    {
        xDim = largestDim;
        yDim = (int)((float)largestDim *
                     ((float)CvUtils::CvHeight(img) / (float)CvUtils::CvWidth(img)));
    }
    else
    {
        yDim = largestDim;
        xDim = (int)((float)largestDim *
                     (int)((float)CvUtils::CvWidth(img) / (float)CvUtils::CvHeight(img)));
    }

    return cv::Vec2i(xDim, yDim);
}

void LoggingTools::ShowImage(std::string name,
                             const cv::Mat &img,
                             const std::vector < cv::Point > &pointFeatures)
{
    ...
    name = name + " (" + std::to_string(CvUtils::CvWidth(img)) + ", " +
           std::to_string(CvUtils::CvHeight(img)) + ")";
    ...
    {
        cv::circle(imgToShow, point, 2, cv::Scalar{ 0, 0, 0 }, 24);      // Nominal
                                                                         // width
                                                                         // was
                                                                         // 24
    }
    ...
    if (logging_tool_wait_for_keypress_)
    {
        ...
        if (keyPressed == 115)        // "s"
        {
            ...
            BOOST_LOG_TRIVIAL(debug) << "Saved to file: " << kDefaultSaveFileName <<
                ". Press any key to continue";
            cv::waitKey(0);      // Give user a chance to move the saved file if
                                 // necessary
        }
    }
}
...
// Save the image (possibly with some pointFeatures) to a timestamped file whose
// name
// will include the fileNameTag.  Example, with fileNameTag = "last_hit":
//      "gs_log_img__last_hit__2023-11-13_12-52-47.0.png"
// If forceFixedFileName is true, the logged image filename will be
// fixedFileName, but.
// still preceded by the default logging directory.
// For example, "last_GetBall_img.png", so that the file keeps getting
// overwritten.
                            ...
                            const std::string &fixedFileName)
{
    ...
    for (auto &point : pointFeatures)
    {
        cv::circle(imgToLog, point, 2, cv::Scalar{ 0, 0, 0 }, 24);      // Nominal
                                                                        // width
                                                                        // was
                                                                        // 24
    }
...
void LoggingTools::DebugShowImage(std::string name, const cv::Mat &img,
                                  const std::vector < cv::Point > &pointFeatures)
{
...
void LoggingTools::ShowRectangleOnImage(std::string name,
...
void LoggingTools::DebugShowColorSwatch(std::string name, cv::Scalar bgr)
{
    cv::Mat img = cv::Mat::zeros(200, 200, CV_8UC3);
    cv::Scalar color{ bgr };
    cv::rectangle(img, cv::Point(0, 0), cv::Point(199,
                                                  199), color, -1);
...
// Creates its own copy of the image, so does not affect the original
void LoggingTools::ShowContours(std::string name, const cv::Mat &baseImage,
...
// Only shows the image if the logging level is at or below debug
void LoggingTools::DebugShowContours(std::string name, const cv::Mat &baseImage,
                                     std::vector<std::vector<cv::Point> > contours)
{
    // TBD - Figure out logging level
...
void LoggingTools::GetOneImage(std::vector<cv::Mat> images)
{
    ...
    int current_y = 0;      // keep track of where your current image was last
                            // placed in the y coordinate
...
void LoggingTools::DrawCircleOutlineAndCenter(cv::Mat &img,
                                              cv::Vec3f circle,
                                              std::string label,
                                              int ordinal,
                                              bool de_emphasize)
{
    ...
    cv::putText(img,
                label,
                cv::Point((int)circle[0] + 2 + ((float)(ordinal) * .2),
                          (int)circle[1] + ((float)(ordinal) * .2)),
...
{
    std::string s = "(sizeX, sizeY) = (" + std::to_string(img.cols) + ", " +
...
std::string LoggingTools::FormatGsColorTriplet(const cv::Scalar &v)
{
    auto f = std::format("[{: >6.2f}, {: >6.2f}, {: >6.2f}]", v[0], v[1], v[2]);
    return f;
}

std::string LoggingTools::FormatCircle(const cv::Vec3f &c)
{
    auto f = std::format( "[(x,y)=({: >4},{: <4}), r={: <6.1f}]", c[0], c[1], c[2] );
    return f;
}

std::string LoggingTools::FormatCircleList(const std::vector<cv::Vec3f> &circleList)
{
    std::string s;
