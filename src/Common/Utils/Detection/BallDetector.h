...
struct BallDetectionConfig
{
    ...
    int required_consecutive_frames = 3;     // Frames needed for confirmed detection
    float max_movement_distance = 50.0f;    // Max pixels ball can move between frames
    ...
    // Tracking settings
    std::chrono::milliseconds max_detection_age{1000}; // Max time to keep detection
...
public:
     ...
     */
    virtual std::vector<BallDetection> detectBalls(const cv::Mat& frame) = 0;
...
public:
    ColorBasedDetection(cv::Scalar lower = cv::Scalar(25, 50, 50),
                       cv::Scalar upper = cv::Scalar(35, 255, 255),
                       float min_r = 10.0f, float max_r = 100.0f);
    
    std::vector<BallDetection> detectBalls(const cv::Mat& frame) override;
    std::string getAlgorithmName() const override { return "ColorBased"; }
    void configure(const cv::FileStorage& config) override;
    
    void setColorRange(cv::Scalar lower, cv::Scalar upper);
    void setSizeRange(float min_radius, float max_radius);
};
...
public:
    TemplateMatchingDetection(const cv::Mat& ball_template, float threshold = 0.8f);
    
    std::vector<BallDetection> detectBalls(const cv::Mat& frame) override;
    std::string getAlgorithmName() const override { return "TemplateMatching"; }
    void configure(const cv::FileStorage& config) override;
    
    void setTemplate(const cv::Mat& ball_template);
...
public:
    ContourBasedDetection(double min_area = 100.0, double max_area = 5000.0,
                         double min_circularity = 0.7, double min_convexity = 0.8);
    
    std::vector<BallDetection> detectBalls(const cv::Mat& frame) override;
    std::string getAlgorithmName() const override { return "ContourBased"; }
    void configure(const cv::FileStorage& config) override;
};
...
public:
     ...
     */
    BallDetector(std::unique_ptr<IBallDetectionAlgorithm> algorithm,
     ...
     */
    bool isBallDetected() const { return ball_detected_flag_.load(); }
    
    /**
     * @brief Clear the ball detection flag
     */
    void clearDetectionFlag() { ball_detected_flag_.store(false); }
    
    /**
     * @brief Get current validated ball detections
     */
    const std::vector<BallDetection>& getValidatedBalls() const { return validated_balls_; }
    
    /**
     * @brief Set detection callback (called on each frame detection)
     */
    void setOnBallDetected(DetectionCallback callback) { on_ball_detected_ = callback; }
    
    /**
     * @brief Set validation callback (called when ball is validated)
     */
    void setOnBallValidated(ValidationCallback callback) { on_ball_validated_ = callback; }
    
    /**
     * @brief Change detection algorithm
     */
    void setAlgorithm(std::unique_ptr<IBallDetectionAlgorithm> algorithm);
    
    /**
     * @brief Update configuration
     */
    void configure(const BallDetectionConfig& config) { config_ = config; }
     ...
     */
    void drawDebugInfo(cv::Mat& frame, bool show_all_detections = false) const;
...
private:
     ...
     */
    bool findMatchingDetection(const BallDetection& current, 
...
public:
    ...
    static std::unique_ptr<IBallDetectionAlgorithm> createAlgorithm(
        AlgorithmType type, const cv::FileStorage& config = cv::FileStorage{});
