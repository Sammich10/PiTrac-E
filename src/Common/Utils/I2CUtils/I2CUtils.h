...
// Assuming we want to store a unique camera ID (UUID) in the EEPROM, we can define the following constants:
// UUID Is stored starting at address 0x00, with a maximum length of 40 bytes (enough for "CAM-" + 32 hex chars + null terminator)
...
public:
    /**
     * @brief Maps a libcamera device tree path to the corresponding I2C bus number
     * 
     * Takes a device tree path like "/base/axi/pcie@1000120000/rp1/i2c@80000"
     * and finds the corresponding /dev/i2c-X device by matching the controller address.
     * 
     ...
     */
    static int mapDeviceTreePathToI2CBus(const std::string& deviceTreePath);
     ...
     */
    static std::vector<int> findAllI2CBusesForController(const std::string& deviceTreePath);
     ...
     */
    static uint8_t extractI2CAddressFromPath(const std::string& deviceTreePath);
     ...
     */
    static std::string getI2CDevicePath(int busNumber);
     ...
     */
    static std::vector<uint8_t> readEEPROM(int busNumber, uint8_t deviceAddr, 
     ...
     */
    static bool writeEEPROM(int busNumber, uint8_t deviceAddr, 
     ...
     */
    static std::string readEEPROMString(int busNumber, uint8_t deviceAddr, 
                                       uint16_t startAddr, size_t maxLength = 32);
     ...
     * @return True if successful, false otherwise
     */
    static bool writeEEPROMString(int busNumber, uint8_t deviceAddr, 
     ...
     */
    static std::string generateAndStoreCameraUID(int busNumber, uint8_t deviceAddr, 
     ...
     */
    static std::string readStoredCameraUID(int busNumber, uint8_t deviceAddr = 0x50);
