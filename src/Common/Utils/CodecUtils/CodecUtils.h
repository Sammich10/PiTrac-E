...
public:
    ...
    virtual std::vector<uint8_t> encode(const cv::Mat& frame, const CodecParams& params = {}) const = 0;
    virtual cv::Mat decode(const std::vector<uint8_t>& data) const = 0;
};

// Specific codec implementations
class JpegCodec : public FrameCodec {
public:
    std::string getCodecName() const override { return "jpeg"; }
    
    std::vector<uint8_t> encode(const cv::Mat& frame, const CodecParams& params = {}) const override {
        std::vector<uint8_t> buffer;
        if (frame.empty()) return buffer;
...
public:
    std::string getCodecName() const override { return "png"; }
