...
namespace PiTrac
{
int CvUtils::CircleRadius(const cv::Vec3f &circle)
{
    return ((int)std::round(circle[2]));
}

cv::Vec2i CvUtils::CircleXY(const cv::Vec3f &circle)
{
    return (cv::Vec2i((int)std::round(circle[0]), (int)std::round(circle[1])));
}

int CvUtils::CircleX(const cv::Vec3f &circle)
{
    return ((int)std::round(circle[0]));
}

int CvUtils::CircleY(const cv::Vec3f &circle)
{
...
cv::Scalar CvUtils::ConvertRgbToHsv(const cv::Scalar &rgb)
{
    cv::Scalar hsvUnscaled = colorsys::rgb_to_hsv(cv::Scalar(rgb[2] / kBYTE_MAX,
                                                             rgb[1] / kBYTE_MAX,
                                                             rgb[0] / kBYTE_MAX));
    cv::Scalar hsv = cv::Scalar(hsvUnscaled[0] * kOpenCvHueMax,
                                hsvUnscaled[1] * kOpenCvSatMax,
                                hsvUnscaled[2] * kOpenCvValMax);

    return (hsv);
}

// Note that the hsv value will be stored in openCV format
cv::Scalar CvUtils::ConvertHsvToRgb(const cv::Scalar &hsv)
{
    cv::Scalar rgbUnscaled = colorsys::hsv_to_rgb(cv::Scalar(hsv[0] / kOpenCvHueMax,
                                                             hsv[1] / kOpenCvSatMax,
                                                             hsv[2] / kOpenCvValMax));
    cv::Scalar rgb = cv::Scalar(rgbUnscaled[2] * kBYTE_MAX, rgbUnscaled[1] * kBYTE_MAX,
                                rgbUnscaled[0] * kBYTE_MAX);

    return rgb;
}

float CvUtils::ColorDistance(const cv::Scalar &rgb1, const cv::Scalar &rgb2)
{
    float rgbDiff = sqrt((float)(pow((double)(rgb1[0] - rgb2[0]),
                                     2.0) + pow((double)(rgb1[1] - rgb2[1]),
                                                2.0) + pow((double)(rgb1[2] - rgb2[2]), 2.0)));
    return (rgbDiff);
}

// The ball color will be an average of the colors near the middle of the
// determined ball
// The returned color is in RGB form
std::vector<cv::Scalar> CvUtils::GetBallColorRgb(const cv::Mat &img, const cv::Vec3f &circle)
{
    ...
    if (r == 0)
    {
        // LoggingTools::Error("CvUtils::GetBallColorRgb called with circle of 0
        // radius.");
        std::vector<cv::Scalar> empty;
        return empty;
    }

    const double BOUNDED_BOX_RADIUS_RATIO = r / 2.0;       // 1.6 (which is
                                                           // almost the whole
                                                           // ball may be
                                                           // resulting in too
                                                           // much averaging)
    int xmin = std::max(0, (int)round(x - BOUNDED_BOX_RADIUS_RATIO));
    int xmax = std::min(CvWidth(img), (int)round(x + BOUNDED_BOX_RADIUS_RATIO + 0.5));        // Somehow,
                                                                                              // the
                                                                                              // box
                                                                                              // otherwise
                                                                                              // seems
                                                                                              ...
                                                                                              // the
                                                                                              // left?
    int ymin = std::max(0, (int)round(y - BOUNDED_BOX_RADIUS_RATIO - 0.5));        // Round
                                                                                   // up
                                                                                   // to
                                                                                   // focus
                                                                                   // more
    ...
    // BOOST_LOG_TRIVIAL(trace) << "GetBall_color: xmin,max, ymin,max = " << "("
    // << std::to_string(xmin) << "," << std::to_string(xmax) << ") : (" <<
    // std::to_string(ymin) << "," << std::to_string(ymax) << ")";

    // LoggingTools::ShowRectangleOnImage("getBallColor area to average: ", img,
    ...
    // BOOST_LOG_TRIVIAL(trace) << "Average is (RGB):" <<
    // LoggingTools::FormatGsColorTriplet(avg_color) << " Median is: " <<
    // LoggingTools::FormatGsColorTriplet(median_color) << " STD is " <<
    // LoggingTools::FormatGsColorTriplet(std_color);
    cv::Scalar average_color_Scalar{ avg_color };
    // LoggingTools::DebugShowColorSwatch("Average Color of  Ball (BGR)",
    // average_color_Scalar );

    // TBD - Not sure whaht we're going to be getting from the statistical
    // calls, above
    cv::Scalar avg_color_vec3f{ (float)avg_color[0], (float)avg_color[1], (float)avg_color[2] };
    cv::Scalar median_color_vec3f{ (float)median_color[0], (float)median_color[1],
                                   (float)median_color[2] };
    cv::Scalar std_color_vec3f{ (float)std_color[0], (float)std_color[1], (float)std_color[2] };

    std::vector<cv::Scalar> results{ avg_color_vec3f, median_color_vec3f, std_color_vec3f };
    return results;
}

cv::Mat CvUtils::GetAreaMaskImage(int resolutionX,
                                  int resolutionY,
                                  int expected_ball_X,
                                  int expected_ball_Y,
                                  int mask_radius,
                                  ...
                                  bool use_square)
{
    cv::Mat initialAreaMaskImage = cv::Mat::zeros(resolutionY, resolutionX, CV_8UC3);
    cv::Mat maskImage;
    ...
    cv::Scalar c1(kBYTE_MAX, kBYTE_MAX, kBYTE_MAX);

    // Make sure we're not drawing outside the image
    if (expected_ball_X + mask_radius > resolutionX)
    {
        mask_radius = resolutionX - expected_ball_X;
        // LoggingTools::Warning("Attempted to draw mask area outside image
        // (1).");
    }
    ...
    if (expected_ball_Y + mask_radius > resolutionY)
    {
        mask_radius = resolutionY - expected_ball_Y;
        // LoggingTools::Warning("Attempted to draw mask area outside image
        // (3).");
    }
    ...
    mask_dimensions = cv::Rect(expected_ball_X - mask_radius,
                               expected_ball_Y - mask_radius,
                               2 * mask_radius,
                               2 * mask_radius);
    ...
    cv::GaussianBlur(area_mask_image_, area_mask_image_, cv::Size(5, 5), 0);      // nominal
                                                                                  // was
                                                                                  // 11x11
    cv::erode(area_mask_image_,
              area_mask_image_,
              cv::getStructuringElement(cv::MORPH_RECT,
                                        cv::Size(3, 3)),
              cv::Point(-1, -1),
              2);
    cv::dilate(area_mask_image_,
               area_mask_image_,
               cv::getStructuringElement(cv::MORPH_RECT,
                                         cv::Size(3, 3)),
               cv::Point(-1, -1),
...
double CvUtils::MetersToFeet(const double &m)
{
    return (3.281 * m);
}

double CvUtils::MetersToInches(const double &m)
{
    return 12 * MetersToFeet(m);
}

double CvUtils::InchesToMeters(const double &i)
{
    return (0.0254 * i);
}
...
bool CvUtils::IsUprightRect(const float &theta)
{
    ...
    if (!isUpright)
    {
        // LoggingTools::Warning("Found non-upright ellipse.  Theta = " +
        // std::to_string(theta));
    }
    return isUpright;
}
...
cv::Mat CvUtils::GetSubImage(const cv::Mat &full_image,
                             cv::Rect &ball_ROI_rectInfull_image,
                             ...
                             cv::Point &offset_full_to_sub)
{
    ...
    {
        finalRoi.x = 0;
        // LoggingTools::Warning("CvUtils::GetSubImage received invalid
        // sub-image parameters.  Corrected");
    }
    ...
    // TBD - Might be better to pass in a reference to avoide the sub-image
    // copy?
...
void BrightnessAndContrastAutoAlgo2(const cv::Mat &bgr_image, cv::Mat &dst)
{
    cv::Mat lab_image;
    ...
    // display the results  (you might also want to see lab_planes[0] before and
    // after).
...
void BrightnessAndContrastAutoAlgo1(const cv::Mat &src, cv::Mat &dst, float clip_hist_percent = 0)
{
    ...
    alpha = (histSize - 1) / inputRange;   // alpha expands current range to
                                           // histsize range
    beta = (float)(-minGray * alpha);             // beta shifts current range
                                                  // so that minGray will go to
                                                  // 0
