...
namespace PiTrac {
    ...
    {
        cv::Scalar hsvUnscaled = colorsys::rgb_to_hsv(cv::Scalar(rgb[2] / kBYTE_MAX, rgb[1] / kBYTE_MAX, rgb[0] / kBYTE_MAX));
        cv::Scalar hsv = cv::Scalar(hsvUnscaled[0] * kOpenCvHueMax, hsvUnscaled[1] * kOpenCvSatMax, hsvUnscaled[2] * kOpenCvValMax);
    ...
    {
        cv::Scalar rgbUnscaled = colorsys::hsv_to_rgb(cv::Scalar(hsv[0] / kOpenCvHueMax, hsv[1] / kOpenCvSatMax, hsv[2] / kOpenCvValMax));
        cv::Scalar rgb = cv::Scalar(rgbUnscaled[2] * kBYTE_MAX, rgbUnscaled[1] * kBYTE_MAX, rgbUnscaled[0] * kBYTE_MAX);
    ...
    {
        float rgbDiff = sqrt((float)(pow((double)(rgb1[0] - rgb2[0]), 2.0) + pow((double)(rgb1[1] - rgb2[1]), 2.0) + pow((double)(rgb1[2] - rgb2[2]), 2.0)));
        return (rgbDiff);
    }
    ...
    std::vector<cv::Scalar> CvUtils::GetBallColorRgb(const cv::Mat &img, const cv::Vec3f &circle)
    {
        ...
        if (r == 0) {
            // LoggingTools::Error("CvUtils::GetBallColorRgb called with circle of 0 radius.");
            std::vector<cv::Scalar> empty;
            return empty;
        }

        const double BOUNDED_BOX_RADIUS_RATIO = r / 2.0;   // 1.6 (which is almost the whole ball may be resulting in too much averaging)
        int xmin = std::max(0, (int)round(x - BOUNDED_BOX_RADIUS_RATIO));
        int xmax = std::min(CvWidth(img), (int)round(x + BOUNDED_BOX_RADIUS_RATIO + 0.5));    // Somehow, the box otherwise seems too far to the left?
        int ymin = std::max(0, (int)round(y - BOUNDED_BOX_RADIUS_RATIO - 0.5));    // Round up to focus more on the better-lit top of the ball
        int ymax = std::min(CvHeight(img), (int)round(y + BOUNDED_BOX_RADIUS_RATIO));

        // BOOST_LOG_TRIVIAL(trace) << "GetBall_color: xmin,max, ymin,max = " << "(" << std::to_string(xmin) << "," << std::to_string(xmax) << ") : (" << std::to_string(ymin) << "," << std::to_string(ymax) << ")";
        ...
        // BOOST_LOG_TRIVIAL(trace) << "Average is (RGB):" << LoggingTools::FormatGsColorTriplet(avg_color) << " Median is: " << LoggingTools::FormatGsColorTriplet(median_color) << " STD is " << LoggingTools::FormatGsColorTriplet(std_color);
        ...
        cv::Scalar median_color_vec3f{ (float)median_color[0], (float)median_color[1], (float)median_color[2] };
    ...
    cv::Mat CvUtils::GetAreaMaskImage(int resolutionX, int resolutionY, int expected_ball_X, int expected_ball_Y, int mask_radius, cv::Rect& mask_dimensions, bool use_square)
    {
    ...
    cv::Mat CvUtils::GetSubImage(const cv::Mat& full_image, cv::Rect& ball_ROI_rectInfull_image, cv::Point& offset_sub_to_full, cv::Point& offset_full_to_sub) {
        ...
        if (ball_ROI_rectInfull_image.tl().x < 0 ) {
            finalRoi.x = 0;
            // LoggingTools::Warning("CvUtils::GetSubImage received invalid sub-image parameters.  Corrected");
        }
