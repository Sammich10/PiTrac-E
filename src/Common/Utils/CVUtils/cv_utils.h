 ...
 *
 * This class provides a collection of static helper functions and constants for common
 * OpenCV tasks, including color conversions, geometric calculations, image manipulation,
 ...
 */
{
     ...
     *
     * The input vector is expected to contain the circle parameters in the format (x, y, radius),
     * where x and y are the coordinates of the circle center, and radius is the circle's radius.
     *
     * @param circle A cv::Vec3f containing the circle parameters (x, y, radius).
    ...
    /**
     * @brief Computes the integer (x, y) coordinates of the center of a circle from its parameters.
     *
    ...
    /**
     * @brief Returns the x-coordinate of the center of a circle represented by a cv::Vec3f.
     *
     * The input vector is expected to contain the circle parameters in the form (x, y, radius).
     *
     * @param circle A cv::Vec3f containing the circle's center coordinates and radius.
    ...
    /**
     * @brief Returns the Y-coordinate of the center of a circle represented by a cv::Vec3f.
     *
     * The input vector should contain the circle parameters in the form (x, y, radius).
     *
     * @param circle A cv::Vec3f containing the circle's center coordinates and radius.
     ...
     *
     * This function extracts the width and height of the input OpenCV matrix (image)
     * and returns them as a cv::Vec2i, where the first element is the width (number of columns)
     ...
     *
     * This function retrieves the number of columns (width) in the provided cv::Mat image.
     *
     ...
     *
     * This function takes a cv::Scalar representing an RGB color (with channels in the order R, G, B)
     * and returns a cv::Scalar containing the corresponding HSV values (with channels in the order H, S, V).
     *
     ...
     *
     * This function takes a cv::Scalar representing a color in HSV format and returns
     ...
     *
     * This function computes the color distance between two colors represented as cv::Scalar objects,
     * typically used for RGB values. The distance is calculated in the RGB color space.
     *
    ...
    /**
     * @brief Extracts the RGB color values of a ball from an image based on the provided circle parameters.
     *
     * This function analyzes the specified region in the input image, defined by the given circle,
     * and returns a vector of cv::Scalar objects representing the RGB color(s) detected within the ball area.
     *
     * @param img The input image (cv::Mat) from which to extract the ball's color.
     * @param circle The circle parameters (cv::Vec3f) representing the ball's position and radius in the image.
     * @return std::vector<cv::Scalar> A vector containing the RGB color(s) of the ball.
     * @note  The ball color will be an average of the colors near the middle of the determined ball. The returned color is in RGB form
     */
     ...
     *
     * This function creates a mask image of the specified resolution, with a masked area
     * (either circular or square) centered at the expected ball coordinates. The mask can be
     ...
     * @param expected_ball_X X-coordinate of the expected ball position (center of mask).
     * @param expected_ball_Y Y-coordinate of the expected ball position (center of mask).
     * @param mask_radius Radius of the mask area (for circle or half-side for square).
     * @param mask_dimensions Output parameter that will contain the bounding rectangle of the mask area.
     * @param use_square If true, creates a square mask; otherwise, creates a circular mask.
    ...
    /**
     * @brief Sets the size of the result_image to match the size of image_to_size.
     *
     * This function resizes or adjusts the dimensions of result_image so that it matches
     * the size (rows and columns) of image_to_size. The contents of result_image may be
     * altered or reallocated to fit the new size.
     *
     * @param image_to_size The reference image whose size will be used as the target size.
