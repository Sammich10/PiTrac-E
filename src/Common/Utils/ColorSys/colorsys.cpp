...
namespace PiTrac {
    ...
    {
        const float r = (float)std::clamp((double)(yiq[0] + (0.948262f * yiq[1])) + (0.624013f * yiq[2]), 0., 1.);
        const float g = (float)std::clamp((double)(yiq[0] - (0.276066f * yiq[1])) - (0.63981f * yiq[2]), 0., 1.);
        const float b = (float)std::clamp((double)(yiq[0] - (1.10545f * yiq[1])) + (1.72986f * yiq[2]), 0., 1.);
    ...
    cv::Scalar colorsys::rgb_to_hls(const cv::Scalar& rgb)
    {
        ...
        const float s = (l <= 0.5) ? ((maxc - minc) / (maxc + minc)) : ((maxc - minc) / ((2.0f - maxc) - minc));
