...
#include <opencv4/opencv2/video/tracking.hpp>  // For optical flow
#include <algorithm>
#include <iostream>
    ...
    )
{
    ...
    try {
        ...
        result.analysis_method = "opencv_optical_flow";        // Simple
                                                               // movement
                                                               // detection
                                                               // using frame
                                                               // differencing
    ...
    )
{
    ...
    try {
            ...
            });        if (result.detected_balls.size() >= 2)
        {
            ...
            if (dt > 0)
            {
                double velocity_x = (dx / dt) * 1000000 * 0.001; // Convert to
                                                                 // m/s
...
void OpenCVImageAnalyzer::SetRadiusLimits(int min_radius, int max_radius)
{
    ...
    LogInfo("Radius limits updated: " + std::to_string(min_radius) + "-" +
...
double OpenCVImageAnalyzer::CalculateConfidence(const BallPosition &position,
                                                const cv::Mat &image) const
{
    ...
    // Combined confidence
    double confidence = radius_confidence * edge_confidence * 0.8;  // Base
                                                                    // confidence
                                                                    // factor
    ...
    const cv::Mat &prev_frame, const cv::Mat &curr_frame)
{
    ...
    try {
        ...
        cv::goodFeaturesToTrack(prev_frame,
                                prev_points,
                                100,
                                0.3,
                                7,
        ...
        if (!prev_points.empty())
        {
            ...
            // Calculate optical flow
            cv::calcOpticalFlowPyrLK(prev_frame, curr_frame, prev_points, next_points, status,
                                     error);
