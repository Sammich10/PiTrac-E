...
#include <vector>
#include <optional>
#include <opencv4/opencv2/core.hpp>  // For cv::Point2f, cv::Vec3d
...
struct TeedBallResult
{
    ...
    std::vector<std::string> debug_info;     // Debug information for
                                             // troubleshooting
...
{
    std::vector<BallPosition> detected_balls;     // Multiple ball positions
                                                  // from strobed image
    std::optional<cv::Vec3d> spin_rates;     // x, y, z rotation rates
                                             // (degrees/second)
    std::optional<cv::Vec3d> velocity_vector;     // 3D velocity vector (m/s)
    double temporal_spacing_us = 0.0;     // Time between ball exposures
                                          // (microseconds)
