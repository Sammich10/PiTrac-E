...
bool ball_flight_camera_event_loop(LibcameraJpegApp &app, cv::Mat &returnImg)
{
    GS_LOG_TRACE_MSG(trace,
                     "ball_flight_camera_event_loop started.  Waiting for external trigger....");
    ...
    GS_LOG_TRACE_MSG(trace,
                     "ball_flight_camera_event_loop started.  Opening Camera at slot: " +
    ...
    // Set the starting time to now, even though we will override it when the
    // first trigger is received
    std::chrono::steady_clock::time_point timeOfFirstTrigger = std::chrono::steady_clock::now();

    // These flags manage state while the sequence of external shutter pulses
    // are
    // processed.
    FlightCameraState state = kWaitingForFirstPrimingPulseGroup;

    // Check here, once, to see if we are going to expect to produce a pre-image
    // for later subtraction
    PiTrac::GolfSimConfiguration::SetConstant(
        "gs_config.ball_exposure_selection.kUsePreImageSubtraction",
        PiTrac::GolfSimCamera::kUsePreImageSubtraction);

    bool return_status = true;

    for (; state != kFinalImageReceived;)
    {
        if (!gs::GolfSimGlobals::PiTrac_running_ || return_status == false)
        {
            return_status = false;
            break;
        }
        ...
        // MJLMODIFIED - Here, we're going to ignore any triggered frames
        // received
        // for a period of time to make sure that the device is ready
        // to receive the 'real' trigger pulse.
        //
        // The background on this is that the Pi GS camera appears to require at
        // least
        // a few XTR trigger pulsees in order to be ready to actually take a
        // picture
        //
        switch (state)
        {
            case kWaitingForFinalImageTrigger: {
                GS_LOG_TRACE_MSG(trace,
                                 "Received Final Image Trigger - Image will be de-queued after next (flush) trigger.");
            ...
            case kWaitingForFinalImageFlush: {
                ...
                GS_LOG_TRACE_MSG(trace,
                                 "About to create Mat frame in kWaitingForFinalImageFlush.  Info.height, width = "
                                 + std::to_string(info.height) +
                                 ", " + std::to_string(info.width) + ". Stride = " +
                                 std::to_string(info.stride));

                // TBD - Need to figure out how to get this picture to be in
                // color again!!
                cv::Mat frame = cv::Mat(info.height, info.width, CV_8UC3, image, info.stride);
                // cv::Mat frame = cv::Mat(info.height, info.width, CV_8U,
                // image, info.stride);

                // PiTrac::LoggingTools::LogImage("", frame, std::vector <
                // cv::Point >{}, true, "Cam2_FinalImageFlush_Image.png");
                ...
                // THE FOLLOWING CREATES A SEGMENTATION FAULT: returnImg =
                // cv::Mat(info.height, info.width, CV_8UC3, image,
                // info.stride);
                // So, that's why the frame is being cloned.
                GS_LOG_TRACE_MSG(trace, "Returning (Final, Strobed) Viewfinder captured image");
                // PiTrac::LoggingTools::LogImage("", returnImg, std::vector <
                // cv::Point >{}, true, "Cam2_Strobed_Image.png");
            ...
            case kWaitingForFirstPrimingPulseGroup: {
                // Start the countdown timer.  During this time, we will just
                // receive and
                // ignore the priminmg pulses
                timeOfFirstTrigger = std::chrono::steady_clock::now();
                GS_LOG_TRACE_MSG(trace,
                                 "Received first (priming) trigger of first priming group.  Ignoring it.");

                // Create a completed request to make sure that the buffer(s)
                // get re-used.
            ...
            case kWaitingForFirstPrimingTimeEnd: {
                ...
                // We have been waiting for some time to get ready for the
                // 'real' trigger after
                // having received one or more priming triggers.  Get ready to
                // take the real
                // picture if we have waited long enough.
                auto timeOfCurrentTrigger = std::chrono::steady_clock::now();
                auto timeLapsed =
                    std::chrono::duration_cast<std::chrono::milliseconds>(timeOfCurrentTrigger -
                                                                          timeOfFirstTrigger).count();

                GS_LOG_TRACE_MSG(trace,
                                 "		Time since last trigger: "+ std::to_string(timeLapsed) +
                                 " ms.");
                ...
                if (timeLapsed < kQuiesceTimeMs)
                {
                    ...
                    // Create a completed request to make sure that the
                    // buffer(s) get re-used.
                ...
                {
                    if (!PiTrac::GolfSimCamera::kUsePreImageSubtraction)
                    {
                        if (!PiTrac::GolfSimCamera::kCameraRequiresFlushPulse)
                        {
                            // If no flush is required, jump straight to the
                            // final state
                            GS_LOG_TRACE_MSG(trace,
                                             "Priming period complete.  Ready for Final Image Trigger and Flush.");
                            state = kWaitingForFinalImageFlush;
                        }
                        else
                        {
                            GS_LOG_TRACE_MSG(trace,
                                             "Priming period complete.  Ready for Final Image Trigger (before flush).");
                    ...
                    {
                        GS_LOG_TRACE_MSG(trace,
                                         "Priming period complete.  Ready for Pre-image Trigger.");
            ...
            case kWaitingForPreImageTrigger: {
                ...
                GS_LOG_TRACE_MSG(trace,
                                 "Received Pre-Image Trigger - Image will be de-queued after next (flush) trigger.");
            ...
            case kWaitingForPreImageFlush: {
                 ...
                 *          uint32_t* image = (uint32_t*)mem[0].data();
                 *
                 *          cv::Mat frame = cv::Mat(info.height, info.width,
                 * CV_8UC3, image, info.stride);
                 *
                 *          // Save the image in memory
            ...
            // This state is not curently used.  Instead, the system goes
            // directly from the pre-message
            // flush to waiting for the final image trigger
            case kWaitingForSecondPrimingPulseGroup: {
                timeOfFirstTrigger = std::chrono::steady_clock::now();
                GS_LOG_TRACE_MSG(trace,
                                 "Received first (priming) trigger of SECOND priming group.  Ignoring it.");
            ...
            case kWaitingForSecondPrimingTimeEnd: {
                ...
                // It takes less time to quiesce for the second set of priming
                // pulses
...
bool still_image_event_loop(LibcameraJpegApp &app, cv::Mat &returnImg)
{
    ...
    for (;;)
    {
        ...
        // GS_LOG_MSG(trace, "   Received camera message: " +
        ...
        if (msg.type == RPiCamApp::MsgType::Timeout)
        {
            GS_LOG_MSG(error, "ERROR: Device timeout detected, attempting a restart.");
            GS_LOG_MSG(error,
                       "		Check to make sure the .yaml file in use by libcamera has a long timeout set, for example,  \"camera_timeout_value_ms\": 10000000,  in the appropriate file.");
            GS_LOG_MSG(error,
                       "			On a Pi 4, check both /usr/local/share/libcamera/pipeline/rpi/vc4/rpi_apps.yaml and /usr/share/libcamera/pipeline/rpi/vc4/rpi_apps.yaml");
            GS_LOG_MSG(error,
                       "			On a Pi 5, check both /usr/local/share/libcamera/pipeline/rpi/pisp/rpi_apps.yaml and /usr/share/libcamera/pipeline/rpi/pisp/rpi_apps.yaml");
            app.StopCamera();
            app.StartCamera();
            continue;
        }
        ...
        // In viewfinder mode, simply run until the timeout. When that happens,
        // switch to
        // capture mode.
        if (app.ViewfinderStream())
        {
            ...
            if (options->timeout &&
                now - start_time >
                std::chrono::milliseconds(options->timeout.get<std::chrono::milliseconds>()))
            {
        ...
        else if (app.StillStream())
        {
            ...
            GS_LOG_TRACE_MSG(trace,
                             "Still image (width, height) = (" + std::to_string(w) + "," +
