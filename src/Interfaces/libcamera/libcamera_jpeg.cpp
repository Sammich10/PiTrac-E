...
bool ball_flight_camera_event_loop(LibcameraJpegApp& app, cv::Mat& returnImg)
{
	...
	PiTrac::GolfSimConfiguration::SetConstant("gs_config.ball_exposure_selection.kUsePreImageSubtraction", 
	...
	for (;state != kFinalImageReceived;)
	{
		...
		case kWaitingForFinalImageFlush: {
			...
			// PiTrac::LoggingTools::LogImage("", frame, std::vector < cv::Point >{}, true, "Cam2_FinalImageFlush_Image.png");
			...
			// PiTrac::LoggingTools::LogImage("", returnImg, std::vector < cv::Point >{}, true, "Cam2_Strobed_Image.png");
