...
{
    // Unless we want REALLY detail information, just tell libcamera to be
    // quiet,
    // even for higher-level logging that libcamera might otherwise want to
    // emit.
...
bool LibCameraInterface::SendCamera2PreImage(const cv::Mat &raw_image)
{
    ...
    /***
     * // We must undistort here, because we are going to immediately send the
     * pre-image and the receiver
     * // may not know what camera (and what distortion matrix) is in use.
     * CameraHardware::CameraModel  camera_model =
     * CameraHardware::PiGSCam6mmWideLens;
     * cv::Mat return_image = undistort_camera_image(raw_image,
     * GsCameraNumber::kGsCamera2, camera_model);
     *
     * // Send the image back to the cam1 system
     * GolfSimIPCMessage
     * ipc_message(GolfSimIPCMessage::IPCMessageType::kCamera2ReturnPreImage);
     * ipc_message.SetImageMat(return_image);
     * GolfSimIpcSystem::SendIpcMessage(ipc_message);
     *
     * // Save the image for later analysis
     * LoggingTools::LogImage("", return_image, std::vector < cv::Point >{},
     * true, "log_cam2_last_pre_image.png");
     ***/
    return true;
}
...
bool WatchForBallMovement(GolfSimCamera &camera, const GolfBall &ball, bool &motion_detected)
{
    ...
    // Setup the camera to watch at a high FPS by reducing the portion of the
    // sensor that will
    // be processed in each frame (cropping)

    // Will be setup when camera is configured for cropping, then is used in the
    // ball-watcher-loop
    RPiCamEncoder app;
    ...
    if (motion_detected)
    {
        ...
        for (auto &it : boost::adaptors::reverse(RecentFrames))
        {
            ...
            frame_information += "Frame " + std::to_string(frameIndex) + ": Framerate = " +
                                 std::to_string(it.frameRate) + "\n";
            ...
            if (mostRecentFrameMat.empty())
            {
                GS_LOG_TRACE_MSG(warning,
                                 "Sequence No. " + std::to_string(it.requestSequence) +
                                 " was empty.");
            }
...
bool ConfigCameraForCropping(GolfBall ball, GolfSimCamera &camera, RPiCamEncoder &app)
{
    ...
    // watching_crop_width & Height defines the size of the cropping window,
    // which will be a sub-region of the
    // camera's full resolution.
    float watching_crop_width = 0;
    float watching_crop_height = 0;

    // If we're trying to get club strike image data, we'll need to expand the
    // image size beyond
    ...
    if (GolfSimClubData::kGatherClubData)
    {
        ...
        GS_LOG_TRACE_MSG(trace,
                         "Setting initial crop width/height for club data to: " +
                         std::to_string(watching_crop_width) + "/" +
                         std::to_string(watching_crop_height) + ".");
    }
    else
    {
    ...
    // One issue here is that the current GS camera will not crop any smaller
    // than 98x88.  So, if the ball is smaller
    // than that, we still have to go with the 98x88 and deal with the smaller
    // ball later within the ROI processing

    // We will later want to ensure the ball is not so small that the inscribed
    // watching area ( for high FPS )
    // is larger than the ball and could pick up unrelated movement outside of
    // the ball.
    // We will deal with this when determining the ROI
    uint largest_inscribed_square_side_length_of_ball =
        (double)(CvUtils::CircleRadius(ball.ball_circle_)) * sqrt(2);

    // Reduce the size of the inscribed square a little bit to ensure the motion
    // detection ROI will be within the ball
    largest_inscribed_square_side_length_of_ball *= 0.9;
    GS_LOG_TRACE_MSG(trace,
                     "largest_inscribed_square_side_length_of_ball is: " +
                     std::to_string(largest_inscribed_square_side_length_of_ball));
    ...
    // If we are not gathering club data, then the cropping window is a fixed
    // size.  And if that size
    ...
    if (!GolfSimClubData::kGatherClubData)
    {
        ...
        {
            GS_LOG_TRACE_MSG(trace,
                             "Decreasing cropping window width because largest ball square side = "
                             + std::to_string(largest_inscribed_square_side_length_of_ball));
        ...
        {
            GS_LOG_TRACE_MSG(trace,
                             "Decreasing cropping window height because largest ball square side = "
                             + std::to_string(largest_inscribed_square_side_length_of_ball));
    ...
    // TBD - After all that, and just for the current GS camera, we'll just set
    // the cropping size to the smalllest possible size (for FPS)
    // and either center the ball within that area, or put the ball in the
    // bottom-right of the
    // viewport if we want to see the club data.

    GS_LOG_TRACE_MSG(trace,
                     "Final crop width/height is: " + std::to_string(watching_crop_width) + "/" +
                     std::to_string(watching_crop_height) + ".");

    // Now determine the cropping window's offset within the full camera
    // resolution image
    // This offset will be based on the position of the ball
    // The cropOffset is where, within the full-resolution image, the top-left
    // corner of the
    // cropping window is.
    ...
    GS_LOG_TRACE_MSG(trace,
                     "Ball location (x,y) is: " + std::to_string(ball_x) + "/" +
                     std::to_string(ball_y) + ".");

    // Assume first is that the ball will be centered in the cropping window,
    // then tweak
    // it next if we're in club strike mode. Club strike imaging may require an
    // offset.
    ...
    // TBD - Not sure why the cropped window has to be skewed like this, but
    // otherwise, the ball
    // is often off-center.
    const float crop_offset_scale_adjustment_x = 1.0;     // 0.99;
    const float crop_offset_scale_adjustment_y = 1.0;     //0.99;

    GolfSimConfiguration::SetConstant("gs_config.motion_detect_stage.kCroppedImagePixelOffsetLeft",
                                      kCroppedImagePixelOffsetLeft);
    GolfSimConfiguration::SetConstant("gs_config.motion_detect_stage.kCroppedImagePixelOffsetUp",
    ...
    GS_LOG_TRACE_MSG(trace,
                     "crop_offset_adjustment (x,y) is: (" +
                     std::to_string(crop_offset_adjustment_x) + ",/" +
                     std::to_string(crop_offset_adjustment_y) + ").");

    // The video resolution is a little different than the still-photo
    // resolution.
    // So scale the center of the ball accordingly.
    float x_scale = crop_offset_scale_adjustment_x;     // NOT IMPLEMENTED YET
                                                        //   ((float)camera.camera_hardware_.video_resolution_x_
                                                        // / (float)camera.camera_hardware_.resolution_x_);
    crop_offset_x = crop_offset_adjustment_x +
                    (x_scale *
                     (camera.camera_hardware_.resolution_x_ -
                      (ball_x + watching_crop_width / 2.0)));

    float y_scale = crop_offset_scale_adjustment_y;     // NOT IMPLEMENTED YET
                                                        //  ((float)camera.camera_hardware_.video_resolution_y_
                                                        // / (float)camera.camera_hardware_.resolution_y_);
    crop_offset_y = crop_offset_adjustment_y +
                    (y_scale *
                     (camera.camera_hardware_.resolution_y_ -
                      (ball_y + watching_crop_height / 2.0)));

    GS_LOG_TRACE_MSG(trace,
                     "Video--to-still scaling factor (x,y) is: " + std::to_string(x_scale) + "/" +
                     std::to_string(y_scale) + ".");
    GS_LOG_TRACE_MSG(trace,
                     "Video resolution (x,y) is: " +
                     std::to_string(camera.camera_hardware_.video_resolution_x_) + "/" +
                     std::to_string(camera.camera_hardware_.video_resolution_y_) + ".");
    GS_LOG_TRACE_MSG(trace,
                     "Initial crop offset (x,y) is: " + std::to_string(crop_offset_x) + "/" +
                     std::to_string(crop_offset_y) + ".");

    // If we're trying to get club images, then skew the cropping so that the
    // ball ends up near the
    // bottom-right such that the the golf ball "watch" ROI will eventually also
    // be
    // all the way at the bottom right (to give more room so see the club)
    if (GolfSimClubData::kGatherClubData)
    {
        crop_offset_x += (0.5 * watching_crop_width - 0.5 *
                          largest_inscribed_square_side_length_of_ball);
        crop_offset_y += (0.5 * watching_crop_height - 0.5 *
                          largest_inscribed_square_side_length_of_ball);
    }

    GS_LOG_TRACE_MSG(trace,
                     "Post-Club-Data crop offset (x,y) is: " + std::to_string(crop_offset_x) + "/" +
                     std::to_string(crop_offset_y) + ".");

    // Check for and correct if the resulting crop window would be outside the
    // full resolution image
    // If we need to correct something, preserve the crop width and correct the
    // offset.
    // NOTE - Camera resolutions are 1 greater than the greatest pixel position
    if ((((camera.camera_hardware_.resolution_x_ - 1) - crop_offset_x) + watching_crop_width) >=
        camera.camera_hardware_.resolution_x_)
    {
    ...
    if ((((camera.camera_hardware_.resolution_y_ - 1) - crop_offset_y) + watching_crop_height) >=
        camera.camera_hardware_.resolution_y_)
    {
    ...
    GS_LOG_TRACE_MSG(trace,
                     "Final (adjusted) crop offset x/y is: " + std::to_string(crop_offset_x) + "/" +
                     std::to_string(crop_offset_y) + ".");
        ...
        )
    {
        ...
        // Don't reset the crop if we don't need to.  It takes time, especially
        // the kernel-based cropping command lines
        return true;
    }
    ...
    // TBD - Note - this entire thing should work on x,y vectors to the extent
    // possible.

    // Determine what the resulting frame rate is in the resulting camera mode
    //  (and confirm the resolution)
    // The camera would have been stopped after we took the first picture, so
    // need re-start for this call
    cv::Vec2i cropped_resolution;
    uint cropped_frame_rate_fps;
    if (!RetrieveCameraInfo(camera.camera_hardware_.camera_number_,
                            cropped_resolution,
                            cropped_frame_rate_fps,
                            true))
    {
    ...
    // For the post processing, we also need to know what portion of the cropped
    // window
    // is of interest in terms of determining ball movement.
    // Unlike the cropping window, Offsets here are from the top-left corner of
    // the cropped window
    // Assume the ball is perfectly round, so the roi is square.  We don't want
    ...
    GS_LOG_TRACE_MSG(trace,
                     "x/y differences between the inscribed aquare and the watching crop are: " +
                     std::to_string(size_difference_x) + "/" + std::to_string(size_difference_y) +
                     ".");

    if (size_difference_x >= 0.0)
    {
        // The cropped area is already fully inside the ball (assuming we're not
        // dealing with club strike data
    ...
    {
        // The cropping area is larger than a square inscribed in the ball
        // circle, so we want to focus the
        // ROI on just the area of that square.
        roi_size_x = largest_inscribed_square_side_length_of_ball * x_scale;
        // Essentially center the ROI within the image, assuming that the ball
        // is centered in the cropping area
        // (which will likely not be the case if we are widening the cropping
        // area for club strike data)
    ...
    if (camera.camera_hardware_.video_resolution_x_ < 0 ||
        camera.camera_hardware_.video_resolution_y_ < 0)
    {
        GS_LOG_TRACE_MSG(error,
                         "camera.camera_hardware_.video_resolution_x_ or _y_ have not been set.  Exiting.");
        return false;
    }
    ...
    GS_LOG_TRACE_MSG(trace,
                     "Final roi x/y offset is: " + std::to_string(roi_offset_x) + "/" +
                     std::to_string(roi_offset_y) + ".");
    GS_LOG_TRACE_MSG(trace,
                     "Final roi x/y size is: " + std::to_string(roi_size_x) + "/" +
                     std::to_string(roi_size_y) + ".");

    GS_LOG_TRACE_MSG(trace,
                     "Final roi width/height is: " + std::to_string(roi_size_x) + "/" +
                     std::to_string(roi_size_y) + ".");
    ...
    // back to full-screen later when we're watching for the ball to first
    // appear..
...
{
    GS_LOG_TRACE_MSG(trace,
                     "DiscoverCameraLocation called for camera_number: " +
                     to_string((int)camera_number));

    // The camera location won't change during the course of a single exceution,
    // so
    // no need to figure this out more than once - re-use the earlier values if
    // we can
    ...
    // Otherwise, go out and search all of the possible places to search for the
    // camera
    ...
    {
        GS_LOG_TRACE_MSG(error,
                         "DiscoverCameraLocation - could not get PITRAC_ROOT environment variable");
        return false;
    }
    ...
    s += "       if  grep imx discover_media.txt > /dev/null;  then  cat discover_result.txt >> " +
         kOutputFileName + ";  fi\n";
    s += "            done\n";
    ...
    {
        GS_LOG_TRACE_MSG(error,
                         "DiscoverCameraLocation - failed to open script file " + script_name);
        return false;
    }
    ...
    // At least currently, we need to make the script file executable before
    // calling it
    ...
    {
        GS_LOG_TRACE_MSG(error,
                         "system(DiscoverCameraLocation) failed.  Return value was: " +
                         std::to_string(cmdResult));
        return false;
    }
    ...
    {
        GS_LOG_TRACE_MSG(error,
                         "DiscoverCameraLocation - failed to open output file " + kOutputFileName);
        return false;
    }
    ...
    try {
        ...
        // If we are not receiving two sets of camera data, then something is
        // wrong.
        ...
        if (new_line_position == (int)string::npos)
        {
            ...
            {
                GS_LOG_TRACE_MSG(error,
                                 "No expected new line found in camera location output.  Missing camera when running in single-pi mode.");
                return false;
            }
            else
            {
                // We have only a single line of information.  Do not need to do
                // anything else
            }
        }
        else
        {
            ...
            // Assume (TBD - Confirm with Pi people) that the camera on camera
            // unit 0
            // (the port nearest the LAN port) will correspond to the first line
            // of the
            ...
            GS_LOG_TRACE_MSG(trace,
                             "DiscoverCameraLocation - relevant line of output file for selected camera was: "
                             + line);
        }

        // Parse out the media and device numbers from what should be the first
        // line of the media-ctl location report
        ...
        GS_LOG_TRACE_MSG(trace,
                         "media_number_str = " + media_number_str + ", device_number_str = " +
                         device_number_str);
                               ...
                               cv::Vec2i &cropping_window_offset)
{
    ...
    GS_LOG_TRACE_MSG(trace,
                     "   cropping_window_size: (width, height) = " +
                     std::to_string(cropping_window_size[0]) + ", " +
                     std::to_string(cropping_window_size[1]) + ".");
    GS_LOG_TRACE_MSG(trace,
                     "   cropping_window_offset: (X, Y) = " +
                     std::to_string(cropping_window_offset[0]) + ", " +
                     std::to_string(cropping_window_offset[1]) + ".");

    std::string mediaCtlCmd = GetCmdLineForMediaCtlCropping(camera,
                                                            cropping_window_size,
                                                            cropping_window_offset);
...
bool ConfigurePostProcessing(const cv::Vec2i &roi_size, const cv::Vec2i &roi_offset )
{
    ...
    GS_LOG_TRACE_MSG(trace,
                     "   roi_size: (width, height) = " + std::to_string(roi_size[0]) + ", " +
                     std::to_string(roi_size[1]) + ".");
    GS_LOG_TRACE_MSG(trace,
                     "   roi_offset: (X, Y) = " + std::to_string(roi_offset[0]) + ", " +
                     std::to_string(roi_offset[1]) + ".");
    ...
    GolfSimConfiguration::SetConstant("gs_config.motion_detect_stage.kRegionThreshold",
                                      kRegionThreshold);
    GolfSimConfiguration::SetConstant("gs_config.motion_detect_stage.kMaxRegionThreshold",
                                      kMaxRegionThreshold);
    ...
    MotionDetectStage::incoming_configuration.use_incoming_configuration = true;  // Don't
                                                                                  // use
                                                                                  // .json
                                                                                  // file
                                                                                  // values
    ...
    MotionDetectStage::incoming_configuration.hskip = kHSkip; // TBD - don't
                                                              // hard code the
                                                              // skip factor
...
{
    GS_LOG_TRACE_MSG(trace,
                     "ConfigureLibCameraOptions.  cropping_window_size: (width, height) = " +
                     std::to_string(cropping_window_size[0]) + ", " +
                     std::to_string(cropping_window_size[1]) + ".");
    ...
    options->no_raw = true;  // See
                             // https://forums.raspberrypi.com/viewtopic.php?t=369927
                             // - cameras won't work unless this is set.
    ...
    if (GolfSimClubData::kGatherClubData)
    {
        ...
        shutter_speed_string =
            std::to_string((int)(GolfSimClubData::kClubImageShutterSpeedMultiplier *
                                 (1. / cropped_frame_rate_fps * 1000000.))) + "us";                                                                          // TBD
                                                                                                                                                             // -
                                                                                                                                                             // should
                                                                                                                                                             // be
                                                                                                                                                             // 1,000,000
    ...
    else
    {
        ...
        shutter_speed_string = std::to_string((int)(1. / cropped_frame_rate_fps * 1000000.)) + "us";   // TBD
                                                                                                       // -
                                                                                                       // should
                                                                                                       // be
                                                                                                       // 1,000,000
    ...
    options->shutter.set(shutter_speed_string);   // TBD - should be 1,000,000
                                                  // for uS setting
    ...
    // On the Pi5, there's no hardware H.264 encoding, so let's try to turn it
    // off entirely
    ...
    // dynamic loading of the motion_detection module.  Instead, hop[efully for
    // speed,
    // we will use a statically-bound motion_detection module.  See
    // ball_watcher.cpp
    // options->post_process_file =
    // LibCameraInterface::kCameraMotionDetectSettings;

    if (!GolfSimClubData::kGatherClubData)
    {
        GS_LOG_TRACE_MSG(trace,
                         "ball_watcher_event_loop will use post-process file (unless overridden in PiTrac_config.json): "
                         + options->post_process_file);
    }
...
std::string GetCmdLineForMediaCtlCropping(const GolfSimCamera &camera,
                                          cv::Vec2i croppedHW,
                                          cv::Vec2i crop_offset_xY)
{
    std::string s;
    ...
    if (!DiscoverCameraLocation(camera.camera_hardware_.camera_number_, media_number,
                                device_number))
    {
    ...
    s += "#!/bin/sh\n";
    s += "if  media-ctl -d \"/dev/media" + std::to_string(media_number) +
         "\" --set-v4l2 \"'imx296 " + std::to_string(device_number) + "-001a':0 [fmt:" + format +
         "/" + std::to_string(croppedHW[0]) + "x" + std::to_string(croppedHW[1]) + " crop:(" +
         std::to_string(crop_offset_xY[0]) + "," + std::to_string(crop_offset_xY[1]) + ")/" +
         std::to_string(croppedHW[0]) + "x" + std::to_string(croppedHW[1]) +
         "]\" > /dev/null;  then  echo -e \"/dev/media" + std::to_string(media_number) +
         "\" > /dev/null; break;  fi\n";
                        ...
                        bool restartCamera)
{
    ...
    if (restartCamera)
    {
        try
        {
            ...
            if (options->Parse(1, argv))
            {
                ...
                // Set the camera number (0 or 1, likely) when we have more than
                // one camera
                options->camera = (camera_number == GsCameraNumber::kGsCamera1 ||
                                   !GolfSimOptions::GetCommandLineOptions().run_single_pi_) ? 0 : 1;
                GS_LOG_TRACE_MSG(trace,
                                 "  Camera options->camera set to camera slot: " +
                                 std::to_string(options->camera));

                // Get the camera open for a moment so that we can read its
                // settings
                app.OpenCamera();
    ...
    int camera_slot_number = (camera_number == GsCameraNumber::kGsCamera1 ||
                              !GolfSimOptions::GetCommandLineOptions().run_single_pi_) ? 0 : 1;
    ...
    double fps = fd_ctrl ==
                 cam->controls().end() ? NAN : (1e6 / fd_ctrl->second.min().get<int64_t>());
...
cv::Mat LibCameraInterface::undistort_camera_image(const cv::Mat &img, const GolfSimCamera &camera)
{
    ...
    {
        GS_LOG_MSG(trace,
                   "undistort_camera_image ignoring camera with no undistortion matrix. Returning original image.");
        return img;
    }
    ...
    GS_LOG_MSG(trace,
               "undistort_camera_image called with camera_number = " +
               std::to_string(camera_number) + ", camera_model = " + std::to_string(camera_model));
    ...
    if (camera.camera_hardware_.is_mono_camera_)
    {
        ...
        cv::initUndistortRectifyMap(cameracalibrationMatrix_,
                                    cameraDistortionVector_,
                                    cv::Mat(),
                                    cameracalibrationMatrix_,
...
bool ConfigCameraForFullScreenWatching(const GolfSimCamera &c)
{
    ...
    {
        GS_LOG_MSG(error,
                   "ConfigCameraForFullScreenWatching called with camera that has no resolution set.");
        return false;
    }

    // Ensure no cropping and full resolution on the camera
    std::string mediaCtlCmd = GetCmdLineForMediaCtlCropping(c, cv::Vec2i(width, height),
                                                            cv::Vec2i(0,
                                                                      0));
    GS_LOG_TRACE_MSG(trace, "mediaCtlCmd = " + mediaCtlCmd);
    int cmdResult = system(mediaCtlCmd.c_str());
...
bool SetLibcameraTuningFileEnvVariable(const GolfSimCamera &camera)
{
    ...
    if (camera.camera_hardware_.is_mono_camera_)
    {
        // If this is a mono camera, than we must use the "mono" tuning file,
    ...
    else if (GolfSimOptions::GetCommandLineOptions().GetCameraNumber() ==
...
LibcameraJpegApp * ConfigureForLibcameraStill(const GolfSimCamera &camera)
{
    ...
    GS_LOG_TRACE_MSG(trace,
                     "ConfigureForLibcameraStill called for camera " +
                     std::to_string((int)camera_number));
    ...
    if (app != nullptr ||
        lci::libcamera_configuration_[hardware_camera_index] ==
        lci::CameraConfiguration::kStillPicture)
    {
    ...
    try
    {
        ...
        if (GolfSimOptions::GetCommandLineOptions().GetCameraNumber() == GsCameraNumber::kGsCamera1)
        {
            ...
            GS_LOG_TRACE_MSG(trace,
                             "Setting camera gain, contrast and shutter time to: " +
                             std::to_string(camera_gain) + ", " + std::to_string(camera_contrast) +
                             ", " + std::to_string(still_shutter_time_uS) + ".");
        }
        else
        {
            // We're working with camera 2, which doesn't normally take still
            // pictures.
            // BUT, we might be doing a calibration shot,
            // and if so, we'll want to adjust the gain & contrast to a lower
            ...
            // TBD - This code seems backward.  But everything is working right
            // now,
            // so let's not change until we can really test it all.
            if (GolfSimOptions::GetCommandLineOptions().system_mode_ ==
                SystemMode::kCamera2Calibrate ||
                GolfSimOptions::GetCommandLineOptions().system_mode_ ==
                SystemMode::kCamera2OnePulseOnly ||
                GolfSimOptions::GetCommandLineOptions().system_mode_ ==
                SystemMode::kCamera2BallLocation ||
                GolfSimOptions::GetCommandLineOptions().system_mode_ ==
            ...
            {
                GS_LOG_TRACE_MSG(trace,
                                 "In SystemMode::kCamera2Calibrate (or similar).  Using DEFAULT gain/contrast for Camera2.");
        ...
        // Assume camera 1 will be at slot 0 in all cases.  Camera 2 will be at
        // slot 1
        ...
        options->denoise = "cdn_off";
        options->immediate = true;  // TBD - Trying this for now.  May have to
                                    // work on white balance too
        options->awb = "indoor"; // TBD - Trying this for now.  May have to work
                                 // on white balance too
        options->nopreview = true;
        options->viewfinder_width = 0;
        options->viewfinder_height = 0;
    ...
    {
        GS_LOG_MSG(error,
                   "ERROR in ConfigureForLibcameraStill: *** " + std::string(e.what()) + " ***");
        return nullptr;
    }
...
{
    GS_LOG_TRACE_MSG(trace,
                     "DeConfigureForLibcameraStill called for hardware_camera_index: " +
                     std::to_string(camera_number));

    if (lci::libcamera_app_[camera_number] == nullptr)
    {
        GS_LOG_TRACE_MSG(error,
                         "DeConfigureForLibcameraStill called, but camera_app was null.  Doing nothing.");
        return false;
    }
    ...
    {
        GS_LOG_TRACE_MSG(warning,
                         "DeConfigureForLibcameraStill called, but camera_app was in the wrong configurations (configure was mis-matched). Camera_number was: "
                         + std::to_string((int)lci::libcamera_configuration_[camera_number]) +
                         " Ignoring.");
    }
    ...
    {
        GS_LOG_MSG(error,
                   "ERROR in DeConfigureForLibcameraStill: *** " + std::string(e.what()) + " ***");
        return false;
    }
...
bool TakeRawPicture(const GolfSimCamera &camera, cv::Mat &img)
{
    ...
    GS_LOG_TRACE_MSG(trace,
                     "TakeRawPicture called for camera_number: " + std::to_string(camera_number));
    ...
    img = PiTrac::LibCameraInterface::undistort_camera_image(initialImg, camera);
...
bool CheckForBall(GolfBall &ball, cv::Mat &img)
{
    ...
    const CameraHardware::CameraModel camera_model =
        (GolfSimOptions::GetCommandLineOptions().GetCameraNumber() ==
         GsCameraNumber::kGsCamera1) ? GolfSimCamera::kSystemSlot1CameraType :
        GolfSimCamera::kSystemSlot2CameraType;
    GolfSimCamera camera;
    camera.camera_hardware_.init_camera_parameters(
        GolfSimOptions::GetCommandLineOptions().GetCameraNumber(),
        camera_model);
...
bool WaitForCam2Trigger(cv::Mat &return_image)
{
    ...
    try
    {
        ...
        // On a two-Pi system, each Pi has just one camera, and that camera will
        // be in slot 0
        // On a single-pi system the one Pi 5 has both cameras.  And Camera 2
        // will be in slot 1
        // because Camera 1 is in slot 0.
        if (GolfSimOptions::GetCommandLineOptions().run_single_pi_)
        {
            GS_LOG_TRACE_MSG(trace,
                             "Running in single-pi mode, so assuming Camera 2 will be at slot 1.");
            options->camera = 1;
        }
        else
        {
            GS_LOG_TRACE_MSG(trace,
                             "Not running in single-pi mode, so assuming Camera 2 will be at slot 0.");
            options->camera = 0;
        }
        ...
        options->denoise = "cdn_off";
        options->nopreview = true;
        // TBD - Currently, we are using the viewfinder stream to take the
        ...
        options->shutter.set("11111us"); // Not actually used for external
                                         // triggering.  Just needs to be set to
                                         // something
        options->info_text = "";
    ...
    // LoggingTools::LogImage("", raw_image, std::vector < cv::Point >{}, true,
    // "InitialRawImageCam2.png");

    // Save the image in memory after un-distorting it for the local camera/lens

    return_image = PiTrac::LibCameraInterface::undistort_camera_image(raw_image, c);
    ...
    if (GolfSimOptions::GetCommandLineOptions().camera_still_mode_ )
    {
        ...
        if (output_fname.empty())
        {
            ...
            GS_LOG_TRACE_MSG(trace,
                             "No output output_filename specified.  Will save picture as: " +
                             output_fname);
        }
...
bool PerformCameraSystemStartup()
{
    ...
    // Setup the Pi Camera to be internally or externally triggered as
    // appropriate

    GS_LOG_TRACE_MSG(trace,
                     "PerformCameraSystemStartup: System Mode: " +
                     std::to_string(GolfSimOptions::GetCommandLineOptions().system_mode_));

    SystemMode mode = GolfSimOptions::GetCommandLineOptions().system_mode_;
    GS_LOG_TRACE_MSG(trace,
                     "PerformCameraSystemStartup called with mode = " + std::to_string(mode));
    ...
    switch (mode)
    {
        ...
        case SystemMode::kTestSpin: {
            if (!GolfSimOptions::GetCommandLineOptions().run_single_pi_)
            {
                std::string trigger_mode_command =
            ...
            else
            {
                ...
                * GolfSimCamera::kSystemSlot1CameraType;
                *  if (camera_model ==
                * CameraHardware::CameraModel::InnoMakerIMX296GS3_6mmM12Lens) {
                *   std::string trigger_mode_command =
                * "$PITRAC_ROOT/ImageProcessing/CameraTools/imx296_trigger 6 0";
                *
                *   GS_LOG_TRACE_MSG(trace, "Camera 1 trigger_mode_command = " +
                * trigger_mode_command);
                *   int command_result = system(trigger_mode_command.c_str());
                *
                *   if (command_result != 0) {
                *       GS_LOG_TRACE_MSG(trace, "system(trigger_mode_command)
                * failed.");
                *       return false;
                *   }
                *  }
                *  else {
                *   GS_LOG_TRACE_MSG(trace, "Running in single-pi mode, so not
                * setting camera triggering (internal or external)
                ...
                ****/
                GS_LOG_TRACE_MSG(trace,
                                 "Running in single-pi mode, so not setting camera 1 triggering (internal or external) programmatically.  Instead, please see the following discussion on how to setup the boot/firmware.config.txt dtoverlays for triggering:  https://forums.raspberrypi.com/viewtopic.php?p=2315464#p2315464.");
            }
        }
        break;
        ...
        case SystemMode::kCamera2TestStandalone: {
            ...
            else
            {
                ...
                {
                    std::string trigger_mode_command =
                        "$PITRAC_ROOT/ImageProcessing/CameraTools/imx296_trigger 4 1";

                    GS_LOG_TRACE_MSG(trace,
                                     "Camera 2 trigger_mode_command = " + trigger_mode_command);
                    int command_result = system(trigger_mode_command.c_str());
                ...
                {
                    GS_LOG_TRACE_MSG(trace,
                                     "Running in single-pi mode, so not setting camera 2 triggering (internal or external) programmatically.  Instead, please see the following discussion on how to setup the boot/firmware.config.txt dtoverlays for triggering:  https://forums.raspberrypi.com/viewtopic.php?p=2315464#p2315464.");
                }
            }

            // Create a camera just for purposes of setting the tuning file
            // variable
            GolfSimCamera camera;
            camera.camera_hardware_.init_camera_parameters(GsCameraNumber::kGsCamera2,
                                                           GolfSimCamera::kSystemSlot2CameraType);
