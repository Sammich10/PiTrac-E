...
bool GsGSProResponse::ParseJson(const std::string &gspro_json_string)
{
    ...
    {
        // For now, return true even if we failed - TBD - need to figure out
        // what garbage at end means
        // from the boost library
        GS_LOG_MSG(error,
                   "GsGSProResponse::ParseJson failed to parse GSPro response: " +
                   std::string(e.what()));
        return true;
    }
    ...
    else
    {
        ...
        GS_LOG_MSG(warning,
                   "GsGSProResponse::ParseJson received unknown player handedness value from GSPro response:"
                   + handedness_str);
    }
    ...
    {
        GS_LOG_MSG(warning,
                   "GsGSProResponse::ParseJson received unknown player club value from GSPro response:"
    ...
    switch (return_code)
    {
        ...
        default: {
            ...
            {
                GS_LOG_MSG(error,
                           "Received unknown return_code response from GSPro: " +
