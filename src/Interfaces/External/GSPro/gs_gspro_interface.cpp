...
GsGSProInterface::GsGSProInterface()
{
    ...
    GolfSimConfiguration::SetConstant("gs_config.PiTraculator_interfaces.GSPro.kGSProConnectPort",
...
bool GsGSProInterface::InterfaceIsPresent()
{
    ...
    // though we will use the port and other settings that are not likely to
    // change.
    ...
    if (!GolfSimOptions::GetCommandLineOptions().gspro_host_address_.empty())
    {
        ...
        GS_LOG_TRACE_MSG(trace,
                         "GsGSProInterface::InterfaceIsPresent - kGSProConnectAddress=" +
...
bool GsGSProInterface::ProcessReceivedData(const std::string received_data)
{
    ...
    if (gspro_response.return_code_ == GsGSProResponse::ReturnCode::kPlayerInformation)
    {
        ...
        {
            GS_LOG_MSG(warning,
                       "Received GSPro unknown club information.  Player_club was: " +
    ...
    {
        GS_LOG_MSG(info,
                   "GsSimSocketInterface::ProcessReceivedData Received unknown GSPro result type.  Result was: \n"
                   + gspro_response.Format());
    }
