...
{
    // The socket interface is basically just a base class, so cannot on it's
    // own ber present
...
bool GsSimSocketInterface::Initialize()
{
    ...
    try
    {
        ...
        GS_LOG_TRACE_MSG(trace,
                         "Connecting to SimSocketServer at address: " + socket_connect_address_ +
                         ":" + socket_connect_port_);
        tcp::resolver::results_type endpoints = resolver.resolve(socket_connect_address_,
                                                                 socket_connect_port_);
        ...
        receiver_thread_ =
            std::unique_ptr<std::thread>(new std::thread(&GsSimSocketInterface::ReceiveSocketData,
                                                         this));
...
void GsSimSocketInterface::ReceiveSocketData()
{
    ...
    while (GolfSimGlobals::PiTrac_running_)
    {
        ...
        try {
            // Read_some should be blocking, but if the socket has closed, it
        ...
        {
            GS_LOG_MSG(error,
                       "GsSimSocketInterface::ReceiveSocketData failed to read from socket - Error was: "
                       + std::string(e.what()));
        }
        ...
        {
            GS_LOG_MSG(warning,
                       "Received 0-length message from server. Will attempt to re-initialize");
        ...
        GS_LOG_TRACE_MSG(trace,
                         "   Read some data (" + std::to_string(len) + " bytes) : " +
        ...
        {
            GS_LOG_MSG(error,
                       "Failed GsSimSocketInterface::ReceiveSocketData - Could process data: " +
                       received_data_string);
            return;
        }
    }
...
bool GsSimSocketInterface::SendResults(const GsResults &results)
{
    ...
    {
        GS_LOG_MSG(error,
                   "GsSimSocketInterface::SendResults called before the interface was intialized.");
        return false;
    }
    ...
    {
        GS_LOG_MSG(error,
                   "GsSimSocketInterface::SendResults called before the interface was intialized - trying to re-initialize.");
        // If we ended the receive thread, try re-initializing the connection
        DeInitialize();
        if (!Initialize())
        {
            GS_LOG_MSG(error,
                       "GsSimSocketInterface::SendResults could not re-intialize thew interface.");
            return false;
        }
    }
    ...
    GS_LOG_TRACE_MSG(trace,
                     "GsSimSocketInterface::SendResult sent " + std::to_string(write_length) +
                     " bytes.");
...
{
    GS_LOG_TRACE_MSG(trace,
                     "GsSimSocketInterface::ProcessReceivedData - No Scoket-based Golf Sim connected to Launch Monitor, so not doing anything with data.  Data was:\n"
                     + received_data);
    return true;
}
}
