...
{
    GolfSimConfiguration::SetConstant("gs_config.PiTraculator_interfaces.kLaunchMonitorIdString",
...
bool GsSimInterface::SimIsConnected()
{
    ...
    {
        // If we're not even initialized, there can't be any connected golf
        // sims.
        return false;
    }
...
bool GsSimInterface::GetSimSystemArmed()
{
    ...
    if (!SimIsConnected())
    {
        // If no sims, then consider the system armed as a reasonable fallback
        // behavior
        return true;
    }
...
bool GsSimInterface::SendResultsToGolfSims(const GsResults &input_results)
{
    ...
    {
        GS_LOG_MSG(warning,
                   "GsSimInterface::SendResultsToGolfSim got out of bounds speed_mph.  Settting to 200.");
...
bool GsSimInterface::SendResults(const GsResults &results)
{
    GS_LOG_TRACE_MSG(trace,
                     "GsSimInterface::SendResults - No Golf Sim connected to Launch Monitor.  Results are: "
                     + results.Format());
    return true;
}
...
{
    GS_LOG_TRACE_MSG(trace,
                     "GsSimInterface::ProcessReceivedData - No Golf Sim connected to Launch Monitor, so not doing anything with data.  Data was:\n"
                     + received_data);
    return true;
}
}
