 ...
 *
 * Provides a common interface for camera operations, calibration, and properties.
 * Derived classes must implement core camera functionality. All derived camera classes
 * are expected to utilize the libcamera library for camera management and frame capture.
 * 
 */
public:

// You can also provide a protected constructor with common parameters
GSCameraInterface(int resX, int resY, float focalLength = 0.0f, TriggerMode mode = TriggerMode::FREE_RUNNING) 
    : resolutionX_(resX), 
    resolutionY_(resY), 
