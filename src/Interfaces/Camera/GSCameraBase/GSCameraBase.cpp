...
cv::Mat GSCameraBase::captureFrame() {
    if (triggerMode_ == TriggerMode::EXTERNAL_TRIGGER) {
        ...
        try {
            ...
            frameCondition_.wait_for(lock, std::chrono::milliseconds(1000), [this] { return frameReady_; });
...
bool GSCameraBase::allocateBuffersForStream(libcamera::Stream* stream) {
    ...
    std::cout << "Allocated " << allocated << " buffers for stream " << static_cast<int>(activeStream_) << std::endl;
...
cv::Mat GSCameraBase::convertBufferToMat(libcamera::FrameBuffer* buffer) {
    ...
    if (streamConfig.pixelFormat == libcamera::formats::SRGGB10_CSI2P) {
        ...
        std::cerr << "Unsupported pixel format: " << streamConfig.pixelFormat.toString() << std::endl;
    }
...
void GSCameraBase::requestComplete(libcamera::Request* request) {
    if (request->status() == libcamera::Request::RequestComplete) {
        // Since we only have one stream active at a time, it's always at index 0
        libcamera::FrameBuffer* buffer = request->findBuffer(config_->at(0).stream());
        if (buffer) {
            std::cout << "Received frame from stream " << static_cast<int>(activeStream_) << std::endl;
...
cv::Mat GSCameraBase::unpack10BitBayer(void* data, int width, int height, size_t stride) {
    ...
    cv::Mat result8bit;
    result.convertTo(result8bit, CV_8UC1, 1.0/4.0);  // Divide by 4 to convert 10-bit to 8-bit
...
bool GSCameraBase::reconfigureForActiveStream() {
    ...
    if (!config_) {
        std::cerr << "Failed to generate configuration for stream " << static_cast<int>(activeStream_) << std::endl;
        return false;
    }

    // Configure the single stream
    libcamera::StreamConfiguration& streamConfig = config_->at(0); // Only one stream now
    ...
    if (ret) {
        std::cerr << "Failed to configure camera for stream " << static_cast<int>(activeStream_) << std::endl;
        return false;
    }
    ...
    // Allocate buffers for the active stream (index 0 since it's the only stream)
