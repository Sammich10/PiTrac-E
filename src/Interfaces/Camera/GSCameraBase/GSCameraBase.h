...
public:
     ...
     *
     * Initializes the camera interface for the IMX296 sensor, setting up sensor dimensions,
     * field of view calculations, and trigger mode. The camera is not configured or capturing
     * upon construction.
     *
     * @param[in] width         Image width in pixels.
     * @param[in] height        Image height in pixels.
     * @param[in] focalLength   Focal length of the lens in millimeters.
     * @param[in] mode          Trigger mode for image acquisition (default: FREE_RUNNING).
     */
    GSCameraBase(int width, int height, float focalLength, TriggerMode mode = TriggerMode::FREE_RUNNING) 
     ...
     *
     * Cleans up resources, stops capturing if necessary, and releases the camera.
     */
    ~GSCameraBase();
     ...
     *
     * @return The next frame as a cv::Mat, or an empty Mat if no frame is available.
     */
     ...
     */
    CAMERA_TYPE getCameraType() const override { return CAMERA_INNOMAKER_IMX296GS; }

    /**
     * @brief Sets the trigger mode for image acquisition.
     *
     * @param[in] mode The desired trigger mode (FREE_RUNNING or EXTERNAL_TRIGGER).
     ...
     * In EXTERNAL_TRIGGER mode, frames are captured upon receiving an external trigger signal.
     *
    ...
    /**
     * @brief Provides a string representation of the camera and its current settings.
     *
...
private:
     ...
     *
     * @param stream Pointer to the libcamera::Stream object for which buffers are to be allocated.
     * 
     ...
     *
     * @return true if the trigger mode was successfully configured; false otherwise.
     */
     ...
     * In FREE_RUNNING mode, this returns the latest frame in the continuous stream.
     *
     * @return The latest frame as a cv::Mat, or an empty Mat if no frame is available.
     */
    cv::Mat getLatestFrame();
    
    /**
     * @brief Get all available frames in the buffer (EXTERNAL_TRIGGER mode only)
     *
    ...
    /**
     * @brief Check if there are frames available in the buffer (EXTERNAL_TRIGGER mode only)
     *
    ...
    /**
     * @brief Get the number of frames currently in the buffer (EXTERNAL_TRIGGER mode only)
     *
    ...
    /**
     * @brief Set the maximum number of frames to buffer (EXTERNAL_TRIGGER mode only)
     *
     ...
     */
    void setMaxFrameBuffer(size_t maxFrames) { maxFrameBuffer_ = maxFrames; }

    /**
     * @brief Get the maximum number of frames that can be buffered (EXTERNAL_TRIGGER mode only)
     *
     * @return The maximum number of frames that can be buffered.
     */
    size_t getMaxFrameBuffer() const { return maxFrameBuffer_; }
