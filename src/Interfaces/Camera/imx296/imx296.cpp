...
bool IMX296Camera::configureTriggerMode(const TriggerMode &mode)
{
    ...
    if (triggerMode_ == TriggerMode::EXTERNAL_TRIGGER) {
        ...
        // Note: Exact control names depend on your libcamera/driver implementation
        controls.set(libcamera::controls::FrameDurationLimits, {INT64_MAX, INT64_MAX}); // No frame rate limit
        // controls.set(libcamera::controls::TriggerMode, 1); // Enable external trigger (if supported)
    } else {
        ...
        // controls.set(libcamera::controls::TriggerMode, 0); // Disable external trigger
    }
