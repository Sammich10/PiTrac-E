...
bool test_hit_trigger() {
    ...
    //    ball.ball_circle_ = cv::Vec3f{ (float)ball.x(), (float)ball.y(), (float)ball.measured_radius_pixels_ };
...
bool TestGSProServer() {
    try
    {
        ...
        GolfSimConfiguration::SetConstant("gs_config.PiTraculator_interfaces.GSPro.kGSProConnectPort", kGSProConnectPort);
...
void run_main(int argc, char* argv[])
{
    ...
    switch (GolfSimOptions::GetCommandLineOptions().system_mode_) {
        ...
        case SystemMode::kTest:
        {
            ...
            std::string address;
            GolfSimConfiguration::SetConstant("gs_config.PiTraculator_interfaces.GSPro.kGSProConnectAddress", address);
