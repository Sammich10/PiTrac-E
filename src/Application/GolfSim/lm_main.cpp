...
// ImageProcessing.cpp { This file contains the main test function. Program
...
void test_image(std::string_view subdir, std::string_view filename)
{
    ...
    if (!color.empty())
    {
        ...
        {
            // For this part of our test suite, if there is no specified color,
            // white(not kUnknown) is our best bet
            ball.ball_color_ = GolfBall::BallColor::kWhite;
        }
    }
    ...
    // Create a single output file with the result overlaid on the original
    // image, along with
     ...
     * ip->final_result_image_.rows, ip->final_result_image_.cols);
     * auto ncCandidatesArray =
     * nc::NdArray<nc::uint8>(ip->candidates_image_.data,
     * ip->candidates_image_.rows, ip->candidates_image_.cols);
...
void test_calibrated_location(std::string twoFootImgName,
                              std::string threeFootImgName,
                              std::string fourFootImgName)
{
    ...
    // Now, test the calibration by seeing if the same ball can be found using
    // the calibrated ball
    ...
    {
        // This original HSV masking range from the calibration is not working
        // well.Try a lower H ?
        // b.ball_hsv_range_ = [(0, 0, 175), (175, 102, 255)]
        GS_LOG_TRACE_MSG(trace, "GET BALL LOCATION FOR 3 FEET");
        img = cv::imread(threeFootImgName);

        // TBD - Consider the fact that the ball should be no larger than the
        // radius of the last found
        // ball, and should have that ball"s average color
        success = c.GetCurrentBallLocation(c, img, b, newBall);
        // TBD _ FIX b.average_color_, b.median_color_, b.std_color_ =
        // cu.GetBallColorRgb(c.img, newBall.ball_circle_)
    }
...
bool testProjection()
{
    ...
    // We prefer the command-line setting even if there's one in the .json
    // config file
    ...
    const std::string k45_DegreeBallFileName_00 = kBaseTestDir +
                                                  "test_ball_masked_45_deg_dulled.png";
...
bool testSpinDetection()
{
     ...
     * "test_ball_spin_strong_landmarks_00d_3280w_bright_00.png";
     * const std::string kUnknown_DegreeBallFileName_00 =
     * "test_ball_spin_strong_landmarks_14d_3280w_bright_00.png";
     *
     * const std::string k0_DegreeBallFileName_00 =
     * "test_ball_spin_strong_landmarks_00d_2592w_bright_01.png";
     * const std::string kUnknown_DegreeBallFileName_00 =
     * "test_ball_spin_strong_landmarks_08d_2592w_bright_01.png";
     *
     * const std::string k0_DegreeBallFileName_00 =
     * "test_ball_spin_strong_landmarks_00d_2592w_bright_00.png";
     * const std::string kUnknown_DegreeBallFileName_00 =
     * "test_ball_spin_strong_landmarks_45d_2592w_bright_00.png";
     *
     * const std::string k0_DegreeBallFileName_00 =
     * k"test_ball_spin_strong_landmarks_00d_2592w_bright_01.png";
     * const std::string kUnknown_DegreeBallFileName_00 =
     * "test_ball_spin_strong_landmarks_35d_2592w_bright_01.png";
     *
     * const std::string k0_DegreeBallFileName_00 =
     * "test_ball_spin_strong_landmarks_00d-x_3280w_bright_02.png";
     * const std::string kUnknown_DegreeBallFileName_00 =
     * "test_ball_spin_strong_landmarks_15d-x_3280w_bright_02.png";
     *
     * const std::string k0_DegreeBallFileName_00 =
     * "test_ball_spin_st-landmarks_rand_1st_3280w_bright_04.png";
     * const std::string kUnknown_DegreeBallFileName_00 =
     * k"test_ball_spin_st-landmarks_rand_2nd_3280w_bright_04.png";
     *
     * const std::string k0_DegreeBallFileName_00 =
     * "test_pos_2592w_BASE6off_22Dist_00inz_00degx_00iny_00.png";
     * const std::string kUnknown_DegreeBallFileName_00 =
     * "test_pos_2592w_6off_22Dist_10inz_30degx_.75iny_00.png";
     *
     * const std::string k0_DegreeBallFileName_00 =
     * "test_ball_spin_strong_landmarks_00d_2592w_bright_00.png";
     * const std::string kUnknown_DegreeBallFileName_00 =
    ...
    // const std::string kUnknown_DegreeBallFileName_00 =
    // "loc_test_20_degree_right_strobed.png";
    // NEEDS TO BE CORRECTED FOR EACH TEST -  Assume only 1 camera for now, so
    // all deltas are 0
    // In meters
    std::vector<cv::Vec3d> camera_positions_from_origin =
        std::vector<cv::Vec3d>({ GolfSimCamera::kCamera1PositionsFromExpectedBallMeters,
    ...
    if (!GsAutomatedTesting::ReadTestImages(k0_DegreeBallFileName_00,
                                            kUnknown_DegreeBallFileName_00,
                                            ball1ImgGray,
                                            ball2ImgGray,
                                            ball1ImgColor,
                                            ball2ImgColor,
    ...
    cv::Vec3d rotationResults = BallImageProc::GetBallRotation(ball1ImgGray,
                                                               ball1,
                                                               ball2ImgGray,
                                                               ball2);
    ...
    // TBD - Now implemented in the GetBallRotation() function.  See how the
    // original image would look if rotated as the GetBallRotation function
    // calculated

    std::string s = "Ball Rotation (degrees):  X: " + std::to_string(rotationResults[0]) + "\tY: " +
                    std::to_string(rotationResults[1]) + "\tZ: " +
                    std::to_string(rotationResults[2]) + "\n";
...
bool TestSpin()
{
    ...
    cv::Vec3d rotationResults = BallImageProc::GetBallRotation(grayImage1, ball1, grayImage2,
                                                               ball2);
#endif
...
{
    // Have to call this here because we are not starting the FSM, but need
    // (simulated) pulse information
    ...
    GolfSimConfiguration::SetConstant("gs_config.testing.kTwoImageTestStrobedImage",
    ...
    if (!GsAutomatedTesting::ReadTestImages(kTestCamImageFileName_00,
                                            kTestCam2StrobedmageFileName,
                                            ball1ImgGray,
                                            ball2ImgGray,
                                            ball1ImgColor,
                                            ball2ImgColor,
...
bool test_strobed_balls_detection()
{
    ...
    if (!GsAutomatedTesting::ReadTestImages(kCam1BallOnTee,
                                            kCam2BallInFlight,
                                            ball1ImgGray,
                                            ball2ImgGray,
                                            ball1ImgColor,
...
bool test_hit_trigger()
{
    ...
    const std::string kStationaryBallFileName_00 = kBaseTestDir +
                                                   "move_test_ball_present_2592w_00.png";
    const std::string kStationaryBallFileName_01 = kBaseTestDir +
                                                   "move_test_ball_present_2592w_01.png";
    const std::string kPreHitCloseBallFileName_00 = kBaseTestDir +
                                                    "move_test_ball_and_club_present_2592w_00.png";
    ...
    c.camera_hardware_.init_camera_parameters(GsCameraNumber::kGsCamera1,
                                              c.camera_hardware_.camera_model_);

    if (!c.prepareToTakePhoto())
    {
        GS_LOG_MSG(error,
                   "Cannot prepare camera for photos");
        return false;
    }
    ...
    //    ball.ball_circle_ = cv::Vec3f{ (float)ball.x(), (float)ball.y(),
...
bool TestExternalSimMessage()
{
    ...
    // If we are interfacing with a TruGolf/E6 system, then we need to make sure
    // that it is armed before
    // sending shot information.  For GSPro, the arming is not important.

    if (GsE6Interface::InterfaceIsPresent())
    {
        GS_LOG_TRACE_MSG(trace,
                         "Sleeping for a while in order have user setup E6 simulator to send 'Arm' message.");
        sleep(15);
    }
    else
    {
...
{
    // Setup a couple of test balls in specific locations.  Each ball needs the
    // same information it would have if
    // the GolfSimCamera::ComputeXyzDistanceFromOrthoCamPerspective function had
    // been called on it
    GolfBall ball1, ball2;
    ...
    if (!c.ComputeXyzDeltaDistances(ball1,
                                    ball2,
...
bool TestGSProServer()
{
    try
    {
        ...
        GolfSimConfiguration::SetConstant(
            "gs_config.PiTraculator_interfaces.GSPro.kGSProConnectPort",
            kGSProConnectPort);
...
void run_main(int argc, char *argv[])
{
    ...
    if (GolfSimOptions::GetCommandLineOptions().send_test_results_)
    {
        ...
        // Send as many test shots as we have to whatever golf sim we are
        // connected to
        ...
        {
            GS_LOG_MSG(info,
                       "********   READY FOR SHOT NO. " + std::to_string(result.shot_number_) +
                       " ********");
         ...
         * ipc_results.result_type_ =
         * GsIPCResultType::kBallPlacedAndReadyForHit;
         * ipc_results.speed_mpers_ = 50.25 + rand() % 40;
         * ipc_results.carry_meters_ = 100 + rand() % 150;
         * ipc_results.launch_angle_deg_ = 12.34;
         * // Tests edge case where a float can be interpreted by MSGPACK as an
         ...
         *
         * ipc_results.log_messages_.push_back("[2024-01-22 11:19:36.321980]
         * (0x000053c0) [debug] [bool __cdecl
         * PiTrac::GolfSimCamera::GetCalibratedBall(const class cv::Mat &,const
         * class cv::Vec<double,3> &,class PiTrac::GolfBall &,const class
         * cv::Vec<int,2>
         * &)(D:\\GolfSim\\LM\\ImageProcessing\\gs_camera.cpp:118)<-bool __cdecl
         * PiTrac::GolfSimCamera::GetCalibratedBall(const class cv::Mat &,const
         * class cv::Vec<double,3> &,class PiTrac::GolfBall &,const class
         * cv::Vec<int,2>
         * &)(D:\\GolfSim\\LM\\ImageProcessing\\gs_camera.cpp:86)...] Looking
         * for a ball with min/max radius (pixels) of: 56, 85");
         * ipc_results.log_messages_.push_back("[2024 - 01 - 22 11:19 :
         * 36.497181](0x000053c0)[trace][void __cdecl
         * PiTrac::LoggingTools::ShowImage(class std::basic_string<char, struct
         * std::char_traits<char>, class std::allocator<char> >, const class
         * cv::Mat&, const class std::vector<class cv::Point_<int>, class
         * std::allocator<class cv::Point_<int> > >
         * &)(D:\\GolfSim\\LM\\ImageProcessing\\logging_tools.cpp:270) < -bool
         * __cdecl PiTrac::GolfSimCamera::GetCalibratedBall(const class
         * cv::Mat&, const class cv::Vec<double, 3> &, class PiTrac::GolfBall&,
         * const class cv::Vec<int, 2>
         * &)(D:\\GolfSim\\LM\\ImageProcessing\\gs_camera.cpp:118)...]
         * ShowImage(AreaMaskImage Photo, (sizeX, sizeY) = (1456, 1088)");
         *
    ...
    if (GolfSimOptions::GetCommandLineOptions().camera_still_mode_)
    {
        ...
        // We will still need IPC and cameras and such to communicate in and
        // between the systems as usual
        if (!PerformSystemStartupTasks())
        {
            GS_LOG_MSG(error, "Failed to PerformSystemStartupTasks.");
            return;
        ...
        {
            GS_LOG_TRACE_MSG(trace,
                             "Running in cam_still_mode on camera1 system.  Will take one picture.");
        }
        else
        {
            GS_LOG_TRACE_MSG(trace,
                             "Running in  cam2 still mode on camera1 system.  Will take one strobed picture in camera2 system.");
        }
        cv::Mat image;
    ...
    switch (GolfSimOptions::GetCommandLineOptions().system_mode_)
    {
        ...
        {
            GS_LOG_MSG(info,
                       "Running in kCamera2 or kCamera2TestStandalone or kRunCam2ProcessForPi1Processing mode.");
        ...
        case SystemMode::kTest:
        {
            ...
            std::string address;
            GolfSimConfiguration::SetConstant(
                "gs_config.PiTraculator_interfaces.GSPro.kGSProConnectAddress",
                address);
        ...
        case SystemMode::kCamera2AutoCalibrate:
        {
            ...
            // Shutdown the camera2 system (if running) so the user doesn't have
            // to do it
            GolfSimIPCMessage ipc_message(GolfSimIPCMessage::IPCMessageType::kShutdown);
            GolfSimIpcSystem::SendIpcMessage(ipc_message);
        ...
        case SystemMode::kCamera2Calibrate:
        {
            ...
            GolfBall ball;
            cv::Mat img;
            // In addition to checking for the ball, this method will send an
            // IPC results
            ...
            for (int i = 0; i < number_attempts; i++)
            {
                ...
                std::string calibration_results_message = "Focal Length = " +
                                                          std::to_string(
                    ball.calibrated_focal_length_) + ".";
        ...
        case SystemMode::kCamera2BallLocation:
        {
            ...
            while (GolfSimGlobals::PiTrac_running_)
            {
                ...
                bool success = camera.GetCalibratedBall(camera,
                                                        img,
                                                        ball,
                                                        search_area_center,
                                                        expectBall);
                ...
                if (success)
                {
                    ...
                    GolfSimCamera::ShowAndLogBalls("kCameraXBallLocation_" + std::to_string(i),
                                                   img,
                                                   balls,
                                                   true);
                ...
                {
                    GS_LOG_MSG(warning,
                               "Unable to find ball.  Check to ensure it is near the kCamera1 of kCamera2PositionsFromExpectedBallMeters and is adequately lit.");
                }

                GolfSimCamera::ShowAndLogBalls("LastAttemptedBallImage_" + std::to_string(i),
                                               img,
                                               empty_balls,
                                               true);
            }
    ...
    switch (GolfSimOptions::GetCommandLineOptions().system_mode_)
    {
        ...
        case SystemMode::kCamera2BallLocation:
        {
            ...
            // TBD - In Windows, this function is mostly just for debugging
            // purposes
            // Now see if the ball location math will come up with a ball in the
            // same place as the
            ...
            GolfSimCamera camera;
            CameraHardware::CameraModel camera_model = (camera_number ==
                                                        GsCameraNumber::kGsCamera1) ?
                                                       GolfSimCamera::kSystemSlot1CameraType :
                                                       GolfSimCamera::kSystemSlot2CameraType;
            camera.camera_hardware_.init_camera_parameters(camera_number, camera_model);
            ...
            GS_LOG_TRACE_MSG(info,
                             "(Simulated) kCamera1BallLocation Sanity Check Returned a ball = " +
                             ball.Format());
     ...
     * // First call will ensure we have the OpenCV library loaded by doing a
     * dummy test
     * for (int i = 0; i < 10; i++) {
     *  test_hit_trigger();
     * }
...
int main(int argc, char *argv[])
{
    try {
        ...
        {
            GS_LOG_MSG(error,
                       "Could not initialize configuration module using config file: " +
        ...
        {
            // Attempt to get the image logging directory from the .json config
            // file
#ifdef __unix__
            GolfSimConfiguration::SetConstant("gs_config.logging.kLinuxBaseImageLoggingDir",
                                              kBaseTestDir);
        ...
        // If the configuration file forgot to add a "/" at the end of the
        // logging directory, we should add it here ourselves
        ...
        GolfSimGlobals::PiTrac_running_ = true;
