...
void GsUISystem::SendIPCErrorStatusMessage(const std::string &error_message)
{
    ...
    if (LoggingTools::current_error_root_cause_ != "")
    {
        ...
        // We've effectively consumed the root cause error, so reset it to empty
        // for
        // any future errors
    ...
    GS_LOG_TRACE_MSG(trace,
                     "FSM is sending an Error-Type IPC Results Message:" + error_result.Format());

    GolfSimIpcSystem::SendIpcMessage(ipc_message);
}

bool GsUISystem::SendIPCStatusMessage(const GsIPCResultType message_type,
                                      const std::string &custom_message)
{
    ...
    switch (message_type)
    {
        case GsIPCResultType::kInitializing:
            results.message_ = "Version 0.0X.  System Mode: " +
        ...
        case GsIPCResultType::kWaitingForBallToAppear:
            if (GolfSimOptions::GetCommandLineOptions().system_mode_ ==
                SystemMode::kCamera1Calibrate ||
                GolfSimOptions::GetCommandLineOptions().system_mode_ ==
                SystemMode::kCamera2Calibrate)
            {
                results.message_ = "Waiting for ball to be teed up at " +
                                   std::to_string(
                    GolfSimCamera::kCamera1CalibrationDistanceToBall) +
                                   "cm in order to perform calibration.";
            }
            else
            {
        ...
        default:
            GS_LOG_TRACE_MSG(trace,
                             "SendIPCStatusMessage received unknown GsIPCResultType : " +
                             std::to_string((int)message_type));
            return false;
            break;
    }
...
void GsUISystem::SendIPCHitMessage(const GolfBall &result_ball,
                                    ...
                                    bool suppress_diagnostic_saving)
{
    ...
    {
        // Save a unique version of the webserver image into a directory that
        // will not get
        // over-written.  A unque timestamp will be added to the file name
        std::string fname(file_name);

        LoggingTools::LogImage(file_name + "_Shot_" +
    ...
    try {
        ...
        {
            GS_LOG_MSG(warning,
                       "GsUISystem::SaveWebserverImage - could not save to file name: " + fname);
        }
    }
                                    ...
                                    bool suppress_diagnostic_saving)
{
    ...
    for (size_t i = 0; i < balls.size(); i++)
    {
        ...
        const cv::Vec3f &c = b.ball_circle_;
