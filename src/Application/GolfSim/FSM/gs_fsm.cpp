...
static void default_signal_handler(int signal_number)
{
    ...
    GolfSimGlobals::PiTrac_running_ = false;
}
...
// This is where the strobed-ball image will be put so that the web interface
// can display it
...
void queueBallStabilizationCheck()
{
    ...
    if (GolfSimGlobals::PiTrac_running_)
    {
    ...
    // No need to restart timer.  The ball stabilizing state will do so if
    // appropriate.
}
...
void setupBallStabilizationCheckTimer()
{
    ...
    {
        BallStabilizationCheckTimerThread =
            new TimedCallbackThread("BallStabilizationCheckTimerThread",
                                    kBallStabilizationTime * 1000,
                                    queueBallStabilizationCheck);
...
void queueCam2ImageReceivedCheck()
{
    ...
    if (GolfSimGlobals::PiTrac_running_)
    {
        GolfSimEventElement checkForCam2ImageReceivedEvent{ new GolfSimEvent::
    ...
    // No need to restart timer.  The rest of the system will do so if
    // appropriate.
}
...
void setupCam2ImageReceivedCheckTimer()
{
    GS_LOG_TRACE_MSG(trace,
                     "setupCam2ImageReceivedCheckTimer - Setting call back for " +
                     std::to_string(kMaxCam2ImageReceivedTimeMs) + " milliseconds.");

    if (ReceivedCam2ImageCheckTimerThread == nullptr)
    {
        ReceivedCam2ImageCheckTimerThread =
            new TimedCallbackThread("setupCam2ImageReceivedCheckTimerThread",
                                    kMaxCam2ImageReceivedTimeMs * 1000,
                                    queueCam2ImageReceivedCheck);
...
{
    GS_LOG_MSG(debug,
               "GolfSim state transition: Initializing - Received Restart - Next state WaitingForSimArmed or WaitingForBall. ");
    ...
    {
        GolfSimEventElement beginWaitingForBallPlacedEvent{ new GolfSimEvent::
...
{
    GS_LOG_MSG(debug,
               "GolfSim state transition: WaitingForBall - Received CheckForCam2ImageReceived ");
...
{
    GS_LOG_MSG(trace,
               "State: WaitingForBall - Received BeginWaitingForBallPlacedEvent - Now waiting for ball to show up.");
    ...
    // That way, we can process other, asynchronous, events like button presses
    // and such as we
    ...
    if (found)
    {
        ...
        std::chrono::steady_clock::time_point lastBallAcquisitionTime =
            std::chrono::steady_clock::now();

        // Schedule the timer for a determined (short) time in the future.  When
        // the timer goes off, an
        // CheckForBallStable event will be injected
        // Create a thread that will queue an event-loop
        // queueBallStabilizationCheck in the future
        setupBallStabilizationCheckTimer();

        // Let the monitor interface know what's happening
        GsUISystem::SendIPCStatusMessage(GsIPCResultType::kPausingForBallStabilization);

        return state::WaitingForBallStabilization{ lastBallAcquisitionTime,
                                                   std::chrono::steady_clock::now(), ball, img };
    }

    // The ball was NOT found.  Report it and then get back into the event loop
    // so
    ...
    // Create an image that the monitor can show the player in order to see
    // where the LM is looking
    ...
    if (ball.search_area_radius_ > 0.01)
    {
        ...
        cv::circle(img,
                   cv::Point(ball.search_area_center_[0], ball.search_area_center_[1]),
                   ball.search_area_radius_,
                   c1,
                   2);
    }

    // Update what it is the LM 'sees' when looking for th teed ball to allow
    // users to see
    // if they need to reposition that ball.
    GsUISystem::SaveWebserverImage(GsUISystem::kWebServerBallSearchAreaImage, img, true);

    // Queue up another event to get back here (after processing any other
    // waiting events)
    GolfSimEventElement newBeginWaitingForBallPlacedEvent{ new GolfSimEvent::
                                                           BeginWaitingForBallPlaced{ } };
    GolfSimEventQueue::QueueEvent(newBeginWaitingForBallPlacedEvent);

    return state::WaitingForBall{ std::chrono::steady_clock::now(),
                                  true /* already_sent_waiting_ipc_message */};
}
...
{
    GS_LOG_MSG(debug,
               "GolfSim state transition: WaitingForBallStabilization - Received CheckForBallStableEvent ");

    GolfBall ball;
    cv::Mat img;

    bool found = CheckForBall(ball, img);
    // LoggingTools::LogImage("", img, std::vector < cv::Point >{}, true,
    // "log_last_ball_2bcompared2_still.png");
    ...
    {
        GS_LOG_MSG(info,
                   "=============== Ball Lost Before Stabilizing - Will look for ball again.");
    }
    ...
    {
        GS_LOG_MSG(info,
                   "=============== Ball Moved (or was lost) Before Stabilizing - Will look for ball again.");

        // This event will cause the WaitingForBall state to begin waiting for
        // the ball to appear teed up again
    ...
    // The sending of the priming pulses will include a trigger to make the
    // camera2
    // take a pre-image.  That will in turn send an event to the camera1 system
    // that
    ...
    // LoggingTools::LogImage("", img, std::vector < cv::Point >{ cv::Vec2i(1456
    // / 2, 1088 / 2) }, true, "log_view_last_ball_found_img.png");
        ...
        ArtifactSaveLevel::kNoArtifacts)
    {
        ...
        {
            // Save a unique version of the webserver image into a directory
            // that will not get
            // over-written.  A unique timestamp will be added to the file name
            LoggingTools::LogImage(kWebServerLastTeedBallImage + "_Shot_" +
                                   std::to_string(GsSimInterface::GetShotCounter()) + "_",
                                   img,
                                   std::vector < cv::Point >{});
        }

        // In any case, save the image with a non-unique name that will be
        // overwritten on the next show, but that the GUI
        // will be able to depend on the name of.
        LoggingTools::LogImageWithCircles("",
                                          img,
                                          std::vector < cv::Vec3f >{ball.ball_circle_},
                                          true,
                                          kWebServerLastTeedBallImage + ".png");
    }
    ...
    // TBD - Probably remove.  Pre-image subtraction was an idea that never
    // panned out as well as we'd hoped.
    ...
    {
        // This even will cause the waitingForBallHit state to begin watching
        // for the hit
...
{
    GS_LOG_MSG(debug,
               "GolfSim state transition: WaitingForCamera2PreImage - Received Camera2PreImageReceived.");

    // This even will cause the waitingForBallHit state to begin watching for
    // the hit
    GolfSimEventElement beginWatchingForBallHit{ new GolfSimEvent::BeginWatchingForBallHit{ } };
    GolfSimEventQueue::QueueEvent(beginWatchingForBallHit);
...
GolfSimState onEvent(const state::WaitingForSimulatorArmed &waitingForSimulatorArmed,
                     const GolfSimEvent::BeginWaitingForSimulatorArmed &
                     beginWaitingForSimulatorArmed)
{
    GS_LOG_MSG(debug,
               "GolfSim state transition: WaitingForSimulatorArmed - Received SimulatorArmed.");
    ...
    GolfSimEventElement nextBeginWaitingForSimulatorArmed{ new GolfSimEvent::
                                                           BeginWaitingForSimulatorArmed{ } };
                     ...
                     const GolfSimEvent::SimulatorIsArmed &simulatorArmed)
{
    ...
    // The following will cause the waitingForBall state to begin watching for
    // the ball
    GolfSimEventElement beginWaitingForSimulatorArmed{ new GolfSimEvent::
                                                       BeginWaitingForSimulatorArmed{ } };
...
{
    GS_LOG_MSG(debug,
               "GolfSim state transition: WaitingForBallHit - Received BeginWatchingForBallHit.");

    // TBD - Figure out a better way to time this.  Need to give camera2 a
    // moment to get ready to
    // receive and process the priming pulses and also probably the
    ...
    // TBD - Consider case where we did NOT get a ball hit indication for some
    // reason
    GS_LOG_MSG(info, "============= BALL HIT ===============\n");

    // Make sure we do something sensible if we don't receive an image from the
    // camera 2
    ...
    return state::BallHitNowWaitingForCam2Image{ waitingForBallHit.cam1_ball_,
                                                 waitingForBallHit.ball_image_,
                                                 waitingForBallHit.camera2_pre_image_ };
}
...
{
    GS_LOG_MSG(debug,
               "GolfSim state transition: BallHitNowWaitingForCam2Image - Received Camera2ImageReceived ");
    ...
    {
        GS_LOG_TRACE_MSG(trace,
                         "Received and processed cam2ImageReceived.  Now sending Results to any connected Golf Simulator");
        ...
        GS_LOG_TRACE_MSG(trace,
                         "Received and processed cam2ImageReceived.  Now sending an IPC Results Message:");

        std::string s;

        float velocity_time_period =
            (float)result_ball.time_between_ball_positions_for_velocity_uS_ / 1000.0;
        auto velocity_time_period_string = GS_FORMATLIB_FORMAT("{: <6.2f}", velocity_time_period);
        s = " Time between chosen images for velocity calculation: " + velocity_time_period_string +
            " ms.";
...
{
    GS_LOG_MSG(debug,
               "GolfSim state transition: BallHitNowWaitingForCam2Image - Received CheckForCam2ImageReceived - Will restart ");

    GS_LOG_MSG(error,
               "BallHitNowWaitingForCam2Image - Timed out waiting for Cam2Image.  Restarting... ");
...
GolfSimState onEvent(const auto &state, const GolfSimEvent::EventLoopTick &eventLoopTick)
{
    ...
    // TBD - At some point, may want to check to make sure we're not shutting
    // down or
...
{
    GS_LOG_MSG(debug,
               "GolfSim state transition: InitializingCamera2System - Received Restart - Next state WaitingForCameraArmMessage. ");
...
{
    GS_LOG_MSG(debug,
               "GolfSim state transition: WaitingForCameraArmMessage - Received ArmCamera2MessageReceived - WAITING FOR EXTERNAL TRIGGER - Next state InitializingCamera2System. ");

    // Prepare for and start waiting for the camera to receive an external
    // trigger
    // and take a picture.
        ...
        ArtifactSaveLevel::kNoArtifacts)
    {
        if (GolfSimCamera::kLogDiagnosticImagesToUniqueFiles)
        {
            ...
            // that will not get
            // over-written.  A unique timestamp will be added to the file name
            // The camera2 system isn't sending message to the simulator system,
            // so we need to update
            // the shot counter here manually
            GsSimInterface::IncrementShotCounter();
            LoggingTools::LogImage(kWebServerCamera2Image + "_Shot_" +
                                   std::to_string(GsSimInterface::GetShotCounter()) + "_",
                                   image,
                                   std::vector < cv::Point >{});
        }

        // In any case, also save the image with a non-unique name that will be
        // overwritten on the next show, but that the GUI
        // will be able to depend on the name of.

        // REMOVE THE TRACE - TBD
        GS_LOG_TRACE_MSG(trace,
                         "Image saving using GolfSimOptions::GetCommandLineOptions().base_image_logging_dir_ = "
                         + GolfSimOptions::GetCommandLineOptions().base_image_logging_dir_);
        LoggingTools::LogImage("", image, std::vector < cv::Point >{}, true,
                               kWebServerCamera2Image);
    }
 ...
 *      GS_LOG_TRACE_MSG(trace, "GolfSim state transition: <Any State> -
 ...
 *      GS_LOG_TRACE_MSG(trace, "GolfSim state transition: Exiting - Received
 * <Any event> - Still Exiting. ");
 *      return state::Exiting{ };
 *  }
 */
...
bool ProcessControlMessageEvent(GolfSimEvent::ControlMessage &control_message)
{
    ...
    GS_LOG_TRACE_MSG(trace,
                     "Processing ControlMessage of type: " +
                     GsIPCControlMsg::FormatControlMessageType(message_type));
...
bool RunGolfSimFsm( const GolfSimState &starting_state )
{
    ...
    GolfSimConfiguration::SetConstant("gs_config.ipc_interface.kMaxCam2ImageReceivedTimeMs",
                                      kMaxCam2ImageReceivedTimeMs);

    GolfSimConfiguration::SetConstant("gs_config.user_interface.kWebServerCamera2Image",
                                      kWebServerCamera2Image);
    GolfSimConfiguration::SetConstant("gs_config.user_interface.kWebServerLastTeedBallImage",
                                      kWebServerLastTeedBallImage);
    ...
    // get the Initializing state to complete and transition to the next, active
    // state.
    ...
    while (GolfSimGlobals::PiTrac_running_)
    {
        ...
        GS_LOG_TRACE_MSG(trace,
                         "       Event Queue size = " +
                         std::to_string(GolfSimEventQueue::GetQueueLength()));
        ...
        try {
            ...
            // If we have been asked to shutdown, set the flag to stop this loop
            // processing
            if (GolfSimEventQueue::EventIsShutdownEvent(eventElement.e_))
            {
                GS_LOG_TRACE_MSG(trace,
                                 "----------- Shutting Down - Received Exit Event -------------");
                GolfSimGlobals::PiTrac_running_ = false;
            }
            else if (GolfSimEventQueue::EventIsControlEvent(eventElement.e_))
            {
                ...
                GolfSimEvent::ControlMessage *control_message =
                    dynamic_cast<GolfSimEvent::ControlMessage *>(eventElement.e_);
        ...
        // If there is another event, we won't pause before processing it in the
        // next loop
    }
...
bool PerformSystemStartupTasks()
{
    ...
    // Even if we are in kCamera2 mode, but also in still mode, we will wan to
    // initialize
    // the GPIO system
    if (GolfSimOptions::GetCommandLineOptions().system_mode_ == SystemMode::kCamera1 ||
