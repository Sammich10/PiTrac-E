 ...
 *
 * This function tests the IMX296 camera interface by performing a series of operations
 * including opening the camera, starting continuous capture, retrieving frames, and stopping capture.
 */
 ...
 int main(int argc, char* argv[]) {
    ...
    // Instantiate the IMX296 camera interface with specific parameters for testing
    const std::unique_ptr<PiTrac::GSCameraInterface> camera = std::make_unique<PiTrac::GSCameraBase>(1456, 1088, 2.8f, PiTrac::TriggerMode::FREE_RUNNING);
    // Test opening the camera
    if (test_open_camera(camera.get(), cameraIndex) != 0) {
        // If opening the camera fails, print an error message and return failure code
