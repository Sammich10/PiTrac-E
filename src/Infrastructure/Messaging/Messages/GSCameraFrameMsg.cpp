...
void GSCameraFrameMessage::serialize(msgpack::sbuffer& buffer) const {
    ...
    // Pack as array: [type, timestamp, camera_id, frame_number, capture_timestamp, fps, frame_data]
    packer.pack_array(7);
...
void GSCameraFrameMessage::serializeMatToBuffer(const cv::Mat& mat, std::vector<uint8_t>& buffer) const {
    if (mat.empty()) {
        buffer.clear();
        return;
    }
