...
public:
    ...
    const cv::Mat& getFrame() const { return frame_; }
    void setFrame(const cv::Mat& frame) { frame_ = frame.clone(); }
    ...
    std::chrono::system_clock::time_point getCaptureTimestamp() const { return capture_timestamp_; }
    ...
    // Utility methods
    cv::Size getFrameSize() const { return frame_.size(); }
    int getFrameType() const { return frame_.type(); }
    bool isEmpty() const { return frame_.empty(); }
