...
#include "Infrastructure/Threading/worker_thread.h"
...
{
    // TBD - Do whatever we need to make sure the timer is not still being
    // waited on.
    repeat_timer_ = false;
    exit_timer_ = true;
...
void TimedCallbackThread::Process()
{
    ...
    while (repeat_timer_ && GolfSimGlobals::PiTrac_running_);
...
{
    while (!m_timerExit && GolfSimGlobals::PiTrac_running_)
    {
    ...
    while (GolfSimGlobals::PiTrac_running_)
    {
        ...
        switch (msg->id)
        {
            case MSG_POST_USER_DATA:
            {
                ...
                std::cout << userData->msg.c_str() << " " << userData->year << " on " <<
