...
// depending on the IPCMessageType.  For example, for kRequestForCamera2Image
// messages,
...
class GolfSimIPCMessage : public activemq::commands::ActiveMQBytesMessage
{
  public:
    ...
    {
        kUnknown = 0,
        kRequestForCamera2Image = 1,     // Sent by the Pi 1 system to signal
                                         // the Pi 2 system that Pi 1 is going
                                         // to expect a picture
        kCamera2Image = 2,        // Sent by the Pi 2 system when it takes a
                                  // picture
        kRequestForCamera2TestStillImage = 3,
        kResults = 4,       // The result of the current system's operation,
                            // such as a ball hit
        kShutdown = 5,      // Tells the system to shutdown and exit
        kCamera2ReturnPreImage = 6,      // Picture of the 'hit' area before the
                                         // ball is actually hit
        kControlMessage = 7        // These are messages coming to the LM from
                                   // outside
    };
