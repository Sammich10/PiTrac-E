...
bool GolfSimIpcSystem::InitializeIPCSystem()
{
    ...
    {
        // At least for now, we will still allow the message broker address to
        // be set
        // from the configuration .json file.
        GolfSimConfiguration::SetConstant("gs_config.ipc_interface.kWebActiveMQHostAddress",
                                          kWebActiveMQHostAddress);
        if (kWebActiveMQHostAddress.empty())
        {
            GS_LOG_TRACE_MSG(error,
                             "GolfSimIpcSystem::InitializeIPCSystem - kWebActiveMQHostAddress not set.  Cannot connect with ActiveMQ system.");
            return false;
        }
    }
    ...
    // allows
    // you to use failover with multiple servers all using certificates signed
    // by
    ...
    if (message_broker_host.empty())
    {
        GS_LOG_TRACE_MSG(trace,
                         "GolfSimIpcSystem could not find host address in JSON config file.");
        return false;
    }

    std::string broker_URI = "failover:(" + message_broker_host + ")" +
                             "?useCompression=true&initialReconnectDelay=2000&maxReconnectAttempts=2";
    //        "?wireFormat=openwire"
    ...
    // Initialization order probably doesn't matter, but we will initialize the
    // consumer first to
...
bool GolfSimIpcSystem::DispatchReceivedIpcMessage(const BytesMessage &message)
{
    ...
    GS_LOG_TRACE_MSG(trace,
                     "DispatchReceivedIpcMessage::Dispatch - message type: " +
                     ipc_message->Format());
...
bool GolfSimIpcSystem::DispatchShutdownMessage(const GolfSimIPCMessage &message)
{
    ...
    // TBD - Not sure if we just want to force the shutdown here or send an
    // event into the
    // FSM and have the FSM do so. Probably better to send the event and get out
    // of this IPC
    // consumer thread callback.

    // This message is telling the system to shutdown and exit
    // Let the FSM deal with the message by entering a related message into the
    // queue
...
bool GolfSimIpcSystem::DispatchControlMsgMessage(const GolfSimIPCMessage &message)
{
    ...
    GolfSimEventElement controlMessageReceived{ new GolfSimEvent::ControlMessage{ message.
                                                                                  GetControlMessage()
                                                                                  .control_type_} };
...
bool GolfSimIpcSystem::DispatchRequestForCamera2TestStillImage(const GolfSimIPCMessage &message)
{
    ...
    // This message is telling the camera 2 system to get ready to take a
    // one-strobe picture, whereas
    // the camera will be externally triggered from the camera 1 system once the
    // ball appears
    // to have been hit.
    // The main difference between this and the usual camera2 picture request is
    // that the
    // TestStillImage will just take one strobe and immediately save it to a
    // file.
    ...
    switch (GolfSimOptions::GetCommandLineOptions().system_mode_)
    {
        ...
        default:
        {
            LoggingTools::Warning(
                "GolfSimIpcSystem::DispatchRequestForCamera2TestStillImage found unknown command_line_options_.system_mode_ .");
            return false;
...
bool GolfSimIpcSystem::DispatchRequestForCamera2ImageMessage(const GolfSimIPCMessage &message)
{
    ...
    switch (GolfSimOptions::GetCommandLineOptions().system_mode_)
    {
        ...
        case SystemMode::kCamera1TestStandalone:
        {
            // A request for the camera 2 system to take a triggered picture has
            // been sent.
            // If we are in test mode for camera 1, camera 2 isn't around, so
            // nothing
        ...
        case SystemMode::kRunCam2ProcessForPi1Processing:
        {
            // Let the FSM deal with the message by entering a related message
            // into the queue
            GolfSimEventElement armCamera2MessageReceived{ new GolfSimEvent::
        ...
        {
            LoggingTools::Warning(
                "GolfSimIpcSystem::DispatchRequestForCamera2ImageMessage found unknown command_line_options_.system_mode_ .");
            return false;
...
bool GolfSimIpcSystem::DispatchCamera2ImageMessage(const GolfSimIPCMessage &message)
{
    ...
    // If in still-image mode, we won't inform the state machine about the
    // message.
    ...
    {
        GS_LOG_TRACE_MSG(trace,
                         "In still-picture, locate or AutoCalibrate camera mode.  Will save received image.");
    ...
    switch (GolfSimOptions::GetCommandLineOptions().system_mode_)
    {
        ...
        case SystemMode::kCamera2TestStandalone:
        {
            // This message is only for the camera 1 system.  Ignore it for
            // camera 2
        ...
        {
            // Let the FSM deal with the message by entering a related message
            // (including the image) into the queue
            GolfSimEventElement cam2ImageMessageReceived{ new GolfSimEvent::Camera2ImageReceived{
                                                              message.GetImageMat() } };
        ...
        {
            LoggingTools::Warning(
                "GolfSimIpcSystem::DispatchCamera2ImageMessage found unknown command_line_options_.system_mode_ .");
            return false;
        }
    }
...
bool GolfSimIpcSystem::DispatchCamera2PreImageMessage(const GolfSimIPCMessage &message)
{
    ...
    switch (GolfSimOptions::GetCommandLineOptions().system_mode_)
    {
        ...
        case SystemMode::kCamera1:
        {
            // Let the FSM deal with the message by entering a related message
            // (including the image) into the queue
            GolfSimEventElement cam2PreImageMessageReceived{ new GolfSimEvent::
                                                             Camera2PreImageReceived{ message.
                                                                                      GetImageMat() } };
        ...
        {
            LoggingTools::Warning(
                "GolfSimIpcSystem::DispatchCamera2PreImageMessage found unknown command_line_options_.system_mode_ .");
            return false;
        }
    }
    ...
    const BytesMessage &active_mq_message)
{
    ...
    try {
        ...
        {
            GS_LOG_TRACE_MSG(trace,
                             "BuildIpcMessageFromBytesMessage received unexpected GolfSimMessageType: "
                             + main_message_type);
            return nullptr;
        }

        GolfSimIPCMessage::IPCMessageType ipc_message_type =
            (GolfSimIPCMessage::IPCMessageType)active_mq_message.getIntProperty(
                kGolfSimIPCMessageTypeTag);
        ...
        GS_LOG_TRACE_MSG(trace,
                         "BuildIpcMessageFromBytesMessage converting Active-MQ message of type " +
                         main_message_type +
                         " and message-type " + std::to_string((int)ipc_message_type) +
                         " to GolfSimIpcMessage");
        ...
        if (ipc_message->GetMessageType() == GolfSimIPCMessage::IPCMessageType::kCamera2Image ||
            ipc_message->GetMessageType() ==
            GolfSimIPCMessage::IPCMessageType::kCamera2ReturnPreImage)
        {
            GS_LOG_TRACE_MSG(trace, "BuildIpcMessageFromBytesMessage about to UnpackMatData.");
            // The ActiveMQ message's Byte body has the serialized data from
            // which
        ...
        {
            GS_LOG_TRACE_MSG(trace,
                             "BuildIpcMessageFromBytesMessage will NOT UnpackMatData for IPCMessageType::kResults.");
            // The ActiveMQ message's Byte body has the serialized data from
            // which
             ...
             * msgpack::unpack(unpacked_result_data, static_cast<const
             * char*>(body_data), number_bytes);
             *
             * auto unpacked_gs_ipc_result =
             * unpacked_result_data.get().as<GsIPCResult>();
             *
             * ipc_message->GetResultsForModification() =
             * unpacked_gs_ipc_result;
             */

            // The caller of getBodyBytes owns the data, so clean it up here
            delete body_data;
        }
        else if (ipc_message->GetMessageType() ==
                 GolfSimIPCMessage::IPCMessageType::kControlMessage)
        {
            ...
            GS_LOG_TRACE_MSG(trace,
                             "Packed IPCMessageType::kControlMessage has length = " +
                             std::to_string(number_bytes));
            ...
            GS_LOG_TRACE_MSG(trace,
                             "Packed control msg type = " + std::to_string(control_msg_type));

            GsIPCControlMsg &msg = ipc_message->GetControlMessageForModification();
            msg.control_type_ = (GsIPCControlMsgType)control_msg_type;

            GS_LOG_TRACE_MSG(trace,
                             "Unpacked IPCMessageType::kControlMessage - message was: " +
                             ipc_message->GetControlMessage().Format());
        ...
        GS_LOG_TRACE_MSG(trace,
                         "BuildIpcMessageFromBytesMessage received an exception.  Stack trace is:");
        e.printStackTrace();
        return nullptr;
    }
...
std::unique_ptr<cms::BytesMessage> GolfSimIpcSystem::BuildBytesMessageObjectFromIpcMessage(
    const GolfSimIPCMessage &ipc_message)
{
    GS_LOG_TRACE_MSG(trace,
                     "BuildBytesMessageObjectFromIpcMessage called with IPC message type =" +
                     std::to_string((int)ipc_message.GetMessageType()));
    ...
    {
        GS_LOG_MSG(error,
                   "System::BuildBytesMessageObjectFromIpcMessage could not get a new BytesMessage.");
        return nullptr;
    }
    ...
    {
        GS_LOG_TRACE_MSG(trace,
                         "GolfSimIpcSystem::BuildBytesMessageObjectFromIpcMessage has image -- setting body data of length = "
                         + std::to_string(image_mat_byte_length));
    ...
    else if (ipc_message.GetMessageType() == GolfSimIPCMessage::IPCMessageType::kResults)
    {
        ...
        GS_LOG_TRACE_MSG(trace,
                         "GolfSimIpcSystem::BuildBytesMessageObjectFromIpcMessage setting body data for GsIPCResults of length = "
                         +
                         std::to_string(serialized_result.size()));

        active_mq_message->setBodyBytes((unsigned char *)serialized_result.data(),
...
bool GolfSimIpcSystem::SendIpcMessage(const GolfSimIPCMessage &ipc_message)
{
    ...
    std::unique_ptr<cms::BytesMessage> activeMQ_message =
        BuildBytesMessageObjectFromIpcMessage(ipc_message);

    if (activeMQ_message == nullptr)
    {
        GS_LOG_MSG(error,
                   "GolfSimIpcSystem::SendIpcMessage failed to create activeMQ_message from the GolfSimIPCMessage.");
        return false;
    }
