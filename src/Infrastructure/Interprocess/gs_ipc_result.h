...
class GsIPCResult
{
  ...
  public:
    ...
    int side_spin_rpm_ = 0;         // Negative is left (counter-clockwise from
                                    // above ball)
    int confidence_ = 0;      // 10 - the results are as confident as the system
                              // can be.  0 - no confidence at all.  Probably an
                              // error occrued. Not fully implemented yet.
...
MSGPACK_ADD_ENUM(PiTrac::GolfSimClubs::GsClubType);
MSGPACK_ADD_ENUM(PiTrac::GsIPCResultType);
