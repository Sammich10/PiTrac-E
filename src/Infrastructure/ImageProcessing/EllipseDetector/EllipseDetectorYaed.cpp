 ...
 * Pattern Recognition, Volume 47, Issue 11, November 2014, Pages 3693-3708,
 * ISSN 0031-3203,
 * http://dx.doi.org/10.1016/j.patcog.2014.05.012.
 * (http://www.sciencedirect.com/science/article/pii/S0031320314001976)
 */

#include "Infrastructure/ImageProcessing/EllipseDetector/EllipseDetectorYaed.h"
...
void CEllipseDetectorYaed::DetectEdges13(cv::Mat &DP, VVP &points_1, VVP &points_3)
{
...
void CEllipseDetectorYaed::DetectEdges24(cv::Mat &DN, VVP &points_2, VVP &points_4 )
{
    // Vector of connected edge points
    VVP contours;
                                            ...
                                            )
{
    ...
    // Uses 4 combinations of parameters. See Table 1 and Sect [3.2.3] of the
    // paper.
    {
        float q1 = data_ij.ra;
        float q3 = data_ik.ra;
    ...
    // Compute reliability
    // this metric is not described in the paper, mostly due to space
    // limitations.
    // The main idea is that for a given ellipse (TD) even if the score is high,
    // the arcs
    // can cover only a small amount of the contour of the estimated ellipse.
    // A low reliability indicate that the arcs form an elliptic shape by
    // chance, but do not underlie
    // an actual ellipse. The value is normalized between 0 and 1.
    // The default value is 0.4.

    // It is somehow similar to the "Angular Circumreference Ratio" saliency
    // criteria
    // as in the paper:
...
// Get the coordinates of the center, given the intersection of the estimated
// lines. See Fig. [8] in Sect [3.2.3] in the paper.
...
#endif
                // If all constraints of the selection strategy have been
                // satisfied,
...
void CEllipseDetectorYaed::RemoveShortEdges(cv::Mat &edges, cv::Mat &clean)
{
    VVP contours;
    ...
    {
        VP &edge = contours[i];
        unsigned szEdge = (unsigned)edge.size();
        ...
        {
            clean.at<uchar>(edge[j]) = (uchar)255;
        }
    }
}

void CEllipseDetectorYaed::PreProcessing(cv::Mat &I,
                                         cv::Mat &DP,
                                         cv::Mat &DN
                                         )
{
...
void CEllipseDetectorYaed::DetectAfterPreProcessing(vector<Ellipse> &ellipses,
                                                    cv::Mat &E,
                                                    Mat1f &PHI)
{
    // Set the image size
    ...
    cv::Mat DP = cv::Mat::zeros(_szImg, E.type());      // arcs along positive
                                                        // diagonal
    cv::Mat DN = cv::Mat::zeros(_szImg, E.type());      // arcs along negative
                                                        // diagonal
    ...
    // Other temporary
    VVP points_1, points_2, points_3, points_4;     //vector of points, one for
                                                    // each convexity class
    unordered_map<uint, EllipseData> centers;       //hash map for reusing
                                                    // already computed
                                                    // EllipseData
...
void CEllipseDetectorYaed::Detect(cv::Mat &I, vector<Ellipse> &ellipses)
{
    PiTrac::LoggingTools::DebugShowImage("CEllipseDetectorYaed::Detect - Input Image I", I);
    ...
    // Initialize temporary data structures
    /*
     * cv::Mat DP = cv::Mat::zeros(_szImg, CV_8S);		// arcs along positive
     * diagonal
     * cv::Mat DN = cv::Mat::zeros(_szImg, CV_8S);		// arcs along negative
     * diagonal
     */
    ...
    // Preprocessing
    // From input image I, find edge point with coarse convexity along positive
    // (DP) or negative (DN) diagonal
    ...
    // PiTrac::LoggingTools::DebugShowImage("DP", DP);
    // PiTrac::LoggingTools::DebugShowImage("DN", DN);
    ...
    PiTrac::LoggingTools::DebugShowImage("out", out);
...
void CEllipseDetectorYaed::ClusterEllipses(vector<Ellipse> &ellipses)
{
    ...
    for (int i = 1; i < iNumOfEllipses; ++i)
    {
        ...
        for (int j = 0; j < sz_clusters; ++j)
        {
            ...
            // Centers
            float Dc = ((e1._xc - e2._xc) * (e1._xc - e2._xc) + (e1._yc - e2._yc) *
                        (e1._yc - e2._yc));
            if (Dc > th_Dc)
            {
                //not same cluster
...
void CEllipseDetectorYaed::DrawDetectedEllipses(Mat3b &output,
                                                vector<Ellipse> &ellipses,
                                                int iTopN,
                                                int thickness)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        Scalar color(0, g, 0);
        ellipse(output,
                Point(cvRound(e._xc), cvRound(e._yc)),
