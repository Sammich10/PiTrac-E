...
// TBD - The separation of responsibilities with the PiTrac_camera needs to be
// clarified.

#ifndef PI_TRAC_IMAGE_PROCESSOR_H
#define PI_TRAC_IMAGE_PROCESSOR_H
...
#include "Common/Utils/colorsys.h"
#include "Common/GolfSim/Ball/golf_ball.h"
...
class BallImageProc
{
  public:
    ...
    // The following are constants that control how the ball spin algorithm and
    // the
    // ball (circle) identification works.  They are set from the configuration
    // .json file
    ...
    // TBD - Some of these are redundant - put 'em all in ball_image_proc or in
    // gs_camera, but not both
    ...
    // TBD - Identifying the "best" circle after doing a rough circle
    // identification
    // may be going away.
    ...
    // This determines which potential 3D angles will be searched for spin
    // processing
    ...
    // The location mask is a total(black or white) mask to subset the image
    // down to just
    // the area(s) that we are interested in
    cv::Mat area_mask_image_;

    // Shows the points of the image that were considered as possibly being the
    // golf ball
    cv::Mat candidates_image_;

    // Shows the ball that was identified with a circle and center point on top
    // of original image
    ...
    // Find a golf ball in the picture - this is the main workhorse of the
    // system.
    // if the baseBallWithSearchParams has color information, that information
    // will be used to search for the ball in the picture
    // The returned balls will have at least the following ball information
    // accurately set:
    //      circle
    // If chooseLargestFinalBall is set true, then even a poorer-matching final
    // ball candidate will be chosen over a smaller, better-scored candidate.
    // If expectingBall is true, then the system will not be as picky when
  ...
  private:
    ...
    // When we create a candidate ball list, the elements of that list include
    // not only
    // the ball, but also the ball identifier(e.g., 1, 2...),
    // as well as information about the difference between the ball's
    // average/median/std color versus the expected color.
...
#endif // PI_TRAC_IMAGE_PROCESSOR_H