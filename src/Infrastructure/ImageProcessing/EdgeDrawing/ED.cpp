       ...
       bool _sumFlag)
{
    ...
    segmentNos = 0;
    segmentPoints.push_back(vector<Point>()); // create empty vector of points
                                              // for segments

    edgeImage = Mat(height, width, CV_8UC1, Scalar(0)); // initialize edge
                                                        // Image//CV_8UC1:8bites
                                                        // Unsign
                                                        // C1:�Ҷ�ͼ��Scalar(0):��ʼ��ֵ
    ...
    {
        GaussianBlur(srcImage, smoothImage, Size(), sigma); // calculate kernel
                                                            // from sigma
    }
       ...
       bool selectStableAnchors)
{
    ...
    if (selectStableAnchors)
    {
        ...
        anchorThresh = 0; // anchorThresh used as zero while computing anchor
                          // points if selectStableAnchors set.
                          // Finding higher number of anchors is OK, because we
                          // have following validation steps in
                          // selectStableAnchors.
        ComputeAnchorPoints();
        anchorThresh = _anchorThresh; // set it to its initial argument value
                                      // for further anchor validation.
        ...
        for (int i = 0; i < width * height; i++)
        {
            ...
            else if (edgeImg[i] == 255)
            {
                ...
                int y = i / width;
                int x = i % width;
                anchorPoints.push_back(Point(x, y)); // push validated anchor
                                                     // point to vector
            }
        }
...
void ED::ComputeGradient()
{
    ...
    for (int i = 1; i < height - 1; i++)
    {
        for (int j = 1; j < width - 1; j++)
        {
            ...
            // (E-D) + (H-F)
            //       gy = com1 - com2 + (G-B) = (H-A) - (C-F) + (G-B) = (F-A) +
            // (G-B) + (H-C)
            //
            // For Sobel
            // Then: gx = com1 + com2 + 2*(E-D) = (H-A) + (C-F) + 2*(E-D) =
            // (C-A) + 2*(E-D) + (H-F)
            //       gy = com1 - com2 + 2*(G-B) = (H-A) - (C-F) + 2*(G-B) =
            // (F-A) + 2*(G-B) + (H-C)
            //
            // For Scharr
            // Then: gx = 3*(com1 + com2) + 10*(E-D) = 3*(H-A) + 3*(C-F) +
            // 10*(E-D) = 3*(C-A) + 10*(E-D) + 3*(H-F)
            //       gy = 3*(com1 - com2) + 10*(G-B) = 3*(H-A) - 3*(C-F) +
            // 10*(G-B) = 3*(F-A) + 10*(G-B) + 3*(H-C)
            //
            // For LSD
            // A B
            // C D
            ...
            switch (op)
            {
                ...
                case SCHARR_OPERATOR:
                    gx = abs(3 * (com1 + com2) + 10 *
                             (smoothImg[i * width + j + 1] - smoothImg[i * width + j - 1]));
                    gy = abs(3 * (com1 - com2) + 10 *
                             (smoothImg[(i + 1) * width + j] - smoothImg[(i - 1) * width + j]));
                    [[fallthrough]];

                case LSD_OPERATOR:
                    // com1 and com2 differs from previous operators, because
                    // LSD has 2x2 kernel
...
void ED::ComputeAnchorPoints()
{
    ...
    for (int i = 2; i < height - 2; i++)
    {
        ...
        if (i % scanInterval != 0)
        {
            start = scanInterval; inc = scanInterval;
        }
    ...
    anchorNos = (int)anchorPoints.size(); // get the total number of anchor
                                          // points
}
...
void ED::JoinAnchorPointsUsingSortedAnchors()
{
    ...
    // Now join the anchors starting with the anchor having the greatest
    // gradient value
    int totalPixels = 0;
...
StartOfWhile:
        ...
        else
        {
...
#if 1
                    /* See if we can erase some pixels from the last chain. This
                     * is for cleanup */
            ...
            if (totalLen > 1)
            {
                ...
                // Copy these chains in the forward direction. Skip the first
                // pixel of the first chain
            ...
            segmentNos++;
            segmentPoints.push_back(vector<Point>()); // create empty vector of
                                                      // points for segments
            ...
            for (int k = 2; k < noChains; k++)
            {
                ...
                if (totalLen >= 10)
                {
...
#if 1
                        /* See if we can erase some pixels from the last chain.
                         * This is for cleanup */
                        int fr = chains[chainNo].pixels[0].y;
                        int fc = chains[chainNo].pixels[0].x;
                    ...
                    segmentPoints.push_back(vector<Point>()); // create empty
                                                              // vector of
                                                              // points for
                                                              // segments
                    segmentNos++;
