...
void EDColor::ComputeGradientMapByDiZenzo()
{
    ...
    for (int i = 1; i < height - 1; i++)
    {
        for (int j = 1; j < width - 1; j++)
        {
            ...
            int gxCh1 = com1 + com2 + 2 *
                        (smooth_L[i * width + j + 1] - smooth_L[i * width + j - 1]);
            int gyCh1 = com1 - com2 + 2 *
                        (smooth_L[(i + 1) * width + j] - smooth_L[(i - 1) * width + j]);
            ...
            int gxCh2 = com1 + com2 + 2 *
                        (smooth_a[i * width + j + 1] - smooth_a[i * width + j - 1]);
            int gyCh2 = com1 - com2 + 2 *
                        (smooth_a[(i + 1) * width + j] - smooth_a[(i - 1) * width + j]);
            ...
            int gxCh3 = com1 + com2 + 2 *
                        (smooth_b[i * width + j + 1] - smooth_b[i * width + j - 1]);
            int gyCh3 = com1 - com2 + 2 *
                        (smooth_b[(i + 1) * width + j] - smooth_b[(i - 1) * width + j]);
#endif
            ...
            double theta = atan2(2.0 * gxy, (double)(gxx - gyy)) / 2; // Gradient
                                                                      // Direction
            int grad = (int)(sqrt(((gxx + gyy) + (gxx - gyy) * cos(2 * theta) + 2 * gxy *
                                   sin(2 * theta)) / 2.0) + 0.5);                                                        // Gradient
                                                                                                                         // Magnitude
#else
            ...
            int grad = (int)(sqrt(gxx * cosTheta * cosTheta + 2 * gxy * sinTheta * cosTheta + gyy *
                                  sinTheta * sinTheta) + 0.5);                                                                   // Gradient
                                                                                                                                 // Magnitude
#endif
...
void EDColor::smoothChannel(uchar *src, uchar *smooth, double sigma)
{
    ...
    {
        GaussianBlur(srcImage, smoothImage, Size(7, 7), 1.5);  // seems to be
                                                               // better?
    }
    else
    {
...
void EDColor::validateEdgeSegments()
{
    ...
    memset(gradImg, 0, sizeof(short) * width * height); // reset gradient Image
                                                        // pixels to zero
...
void EDColor::testSegment(int i, int index1, int index2)
{
    ...
    for (int k = index1; k <= index2; k++)
    {
        ...
        {
            minGrad = gradImg[r * width + c]; minGradIndex = k;
        }
    } //end-for
...
void EDColor::extractNewSegments()
{
    ...
    for (int i = 0; i < (int)segments.size(); i++)
    {
        ...
        while (start < (int)segments[i].size())
        {
            ...
            if (len >= 10)
            {
                // A new segment. Accepted only only long enough (whatever that
                // means)
