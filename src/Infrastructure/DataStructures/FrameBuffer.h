...
/**
 * @class FrameBuffer
 * @brief A circular buffer for storing a fixed number of video frames (cv::Mat).
 *
 * The FrameBuffer class implements a circular buffer to efficiently store and manage
 * a sequence of frames. When the buffer reaches its capacity, adding a new frame
 * will overwrite the oldest frame. The buffer supports adding frames, retrieving
 * frames in FIFO order, and querying its state (empty, full, size, capacity).
 *
 * Usage:
 * - Construct with a positive capacity.
 * - Use addFrame() to insert frames; if the buffer is full, the oldest frame is overwritten.
 * - Use getFrame() to retrieve and remove the oldest frame.
 * - Use isEmpty(), isFull(), size(), and capacity() to query buffer status.
 *
 * @note Thread Safety: This class is not thread-safe. The intended use is a single-producer, single-consumer scenario. 
 *
 * @note Frames are stored as deep copies (using cv::Mat::clone()) to avoid shared data issues.
 */
class FrameBuffer
{
  public:
     ...
     * @return true if the frame was added without overwriting an existing frame,
     *         false if the oldest frame was overwritten to make space for the new frame.
     */
     ...
     *
     * If the buffer is not empty, this function copies the frame at the current tail position
     * into the provided cv::Mat reference, advances the tail index, and returns true.
     * If the buffer is empty, the function returns false and does not modify the frame.
     *
     * @param frame Reference to a cv::Mat object where the retrieved frame will be stored.
     * @return true if a frame was successfully retrieved; false if the buffer is empty.
     */
     ...
     *
     * This function determines whether the buffer has reached its maximum capacity.
     * It returns true if adding another element would overwrite the oldest element,
