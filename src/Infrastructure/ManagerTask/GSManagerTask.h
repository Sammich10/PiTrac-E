...
public:
    ...
    virtual void configureManager() = 0;       // Create and configure the manager
    virtual void configureMessaging() = 0;     // Set up messaging infrastructure
    
    // Manager management
    void setManager(std::unique_ptr<GSManagerInterface> manager);
    GSManagerInterface* getManager() const { return manager_.get(); }
    ...
    std::shared_ptr<GSMessagerBase> getInputMessager() const { return input_messager_; }
    std::shared_ptr<GSMessagerBase> getOutputMessager() const { return output_messager_; }
    
    // Manager task configuration
    void setHealthCheckInterval(std::chrono::milliseconds interval) { 
        health_check_interval_ = interval; 
    }
    void setAutoRestartOnFailure(bool restart) { auto_restart_on_failure_ = restart; }
...
protected:
    ...
    virtual bool preManagerStartHook() { return true; }   // Called before starting manager
    virtual void postManagerStartHook() {}                // Called after manager started
    virtual void onManagerFailedHook() {}                 // Called when manager fails
    virtual void managerMonitoringLoopHook() {}           // Called in each monitoring loop iteration
